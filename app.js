...
/* ===== V31 PATCH · Market Data Foundation ===== */
const V31_APP_LABEL='V31 · Market Data Foundation';
const V31_MARKET_DB='tradingResearchMarket_v31';
const V31_MARKET_DB_VERSION=1;
const V31_MARKET_DATASETS='datasets';
const V31_MARKET_CHUNKS='chunks';
let _v31MarketDbPromise=null;
let v31PendingMarketFile=null;
let v31MarketCache={datasets:[],loaded:false,loading:false,importStatus:null};

/*
  V31 keeps historical market data OUTSIDE the workspace payload.
  Tick/minute files can be large, so raw market data lives only in IndexedDB in this browser.
  Supabase/Conflict Guard keeps synchronizing the journal exactly as before.
*/
function v31MarketDb(){
  if(_v31MarketDbPromise)return _v31MarketDbPromise;
  _v31MarketDbPromise=new Promise((resolve,reject)=>{
    const req=indexedDB.open(V31_MARKET_DB,V31_MARKET_DB_VERSION);
    req.onupgradeneeded=()=>{
      const db=req.result;
      if(!db.objectStoreNames.contains(V31_MARKET_DATASETS))db.createObjectStore(V31_MARKET_DATASETS,{keyPath:'id'});
      if(!db.objectStoreNames.contains(V31_MARKET_CHUNKS)){
        const s=db.createObjectStore(V31_MARKET_CHUNKS,{keyPath:'id'});
  ...
  return _v31MarketDbPromise;
}
function v31IdbReq(req){return new Promise((resolve,reject)=>{req.onsuccess=()=>resolve(req.result);req.onerror=()=>reject(req.error);});}
async function v31DatasetPut(meta){const db=await v31MarketDb(),tx=db.transaction(V31_MARKET_DATASETS,'readwrite');tx.objectStore(V31_MARKET_DATASETS).put(meta);return new Promise((res,rej)=>{tx.oncomplete=res;tx.onerror=()=>rej(tx.error);tx.onabort=()=>rej(tx.error);});}
async function v31ChunkPut(chunk){const db=await v31MarketDb(),tx=db.transaction(V31_MARKET_CHUNKS,'readwrite');tx.objectStore(V31_MARKET_CHUNKS).put(chunk);return new Promise((res,rej)=>{tx.oncomplete=res;tx.onerror=()=>rej(tx.error);tx.onabort=()=>rej(tx.error);});}
async function v31DatasetList(){const db=await v31MarketDb();return v31IdbReq(db.transaction(V31_MARKET_DATASETS,'readonly').objectStore(V31_MARKET_DATASETS).getAll());}
async function v31DatasetGet(id){const db=await v31MarketDb();return v31IdbReq(db.transaction(V31_MARKET_DATASETS,'readonly').objectStore(V31_MARKET_DATASETS).get(id));}
async function v31DeleteDataset(id){
  if(!confirm('¿Eliminar este histórico local de Market Data? No modifica operaciones ni Supabase.'))return;
  const db=await v31MarketDb();
  await new Promise((resolve,reject)=>{
    const tx=db.transaction([V31_MARKET_DATASETS,V31_MARKET_CHUNKS],'readwrite');
    tx.objectStore(V31_MARKET_DATASETS).delete(id);
    const idx=tx.objectStore(V31_MARKET_CHUNKS).index('datasetId'),req=idx.openCursor(IDBKeyRange.only(id));
    req.onsuccess=()=>{const c=req.result;if(c){c.delete();c.continue();}};
    tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error);
  });
  await v31RefreshMarketCache(true);
}
async function v31DeleteDatasetSilent(id){
  const db=await v31MarketDb();
  return new Promise((resolve,reject)=>{
    const tx=db.transaction([V31_MARKET_DATASETS,V31_MARKET_CHUNKS],'readwrite');
    tx.objectStore(V31_MARKET_DATASETS).delete(id);
    const req=tx.objectStore(V31_MARKET_CHUNKS).index('datasetId').openCursor(IDBKeyRange.only(id));
    req.onsuccess=()=>{const c=req.result;if(c){c.delete();c.continue();}};
    tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error);
  });
}
async function v31RefreshMarketCache(rerender=false){
  if(v31MarketCache.loading)return;
  v31MarketCache.loading=true;
  try{v31MarketCache.datasets=(await v31DatasetList()).sort((a,b)=>Number(b.endMs||0)-Number(a.endMs||0));v31MarketCache.loaded=true;}
  catch(e){console.warn('V31 Market Data:',e);}
  finally{v31MarketCache.loading=false;if(rerender&&currentView==='market')render();}
}

function v31NormContract(v){return String(v||'').toUpperCase().trim().replace(/\s+/g,' ').replace(/[–—]/g,'-');}
function v31RootSymbol(v){const s=v31NormContract(v);return (s.match(/^[A-Z0-9]+/)||[''])[0];}
function v31InferFile(file){
  const clean=String(file?.name||'').replace(/\.txt$/i,''),m=clean.match(/^(.*)\.(Last|Bid|Ask)$/i);
  return {sourceContract:(m?m[1]:clean).trim(),dataType:m?m[2][0].toUpperCase()+m[2].slice(1).toLowerCase():'Unknown'};
}
function v31ParseNtTimestamp(raw){
  const s=String(raw||'').trim(),m=s.match(/^(\d{4})(\d{2})(\d{2})(?:\s+(\d{2})(\d{2})(\d{2})(?:\s+(\d{1,7}))?)?$/);
  if(!m)return null;
  const y=+m[1],mo=+m[2]-1,d=+m[3],h=+(m[4]||0),mi=+(m[5]||0),se=+(m[6]||0),frac=(m[7]||'').padEnd(7,'0').slice(0,7),ms=frac?Math.floor(Number(frac)/10000):0;
  const t=Date.UTC(y,mo,d,h,mi,se,ms);return Number.isFinite(t)?t:null;
}
function v31DetectNtFormat(parts){
  const ts=String(parts?.[0]||'').trim();
  if(parts.length===3&&v31ParseNtTimestamp(ts)!==null)return 'tick';
  if(parts.length===5&&v31ParseNtTimestamp(ts)!==null)return 'tickReplay';
  if(parts.length===6&&/^\d{8}\s+\d{6}/.test(ts))return 'minute';
  if(parts.length===6&&/^\d{8}$/.test(ts))return 'day';
  return '';
}
function v31ParseNtLine(line,formatHint=''){
  const s=String(line||'').trim();if(!s)return null;
  const p=s.split(';').map(x=>x.trim()),format=formatHint||v31DetectNtFormat(p);if(!format)return null;
  const t=v31ParseNtTimestamp(p[0]);if(t===null)return null;
  const nums=p.slice(1).map(Number);if(nums.some(x=>!Number.isFinite(x)))return null;
  if(format==='tick')return {format,row:[t,nums[0],nums[1]]};
  if(format==='tickReplay')return {format,row:[t,nums[0],nums[1],nums[2],nums[3]]};
  if(format==='minute'||format==='day')return {format,row:[t,nums[0],nums[1],nums[2],nums[3],nums[4]]};
  return null;
}
function v31FormatLabel(f){return f==='tick'?'Tick':f==='tickReplay'?'Tick Replay':f==='minute'?'1 minuto':f==='day'?'1 día':'Desconocido';}
function v31DataTypeLabel(d){return d==='Replay'?'Last + Bid/Ask':d||'—';}
function v31FormatUtc(ms){if(!Number.isFinite(Number(ms)))return '—';return new Date(Number(ms)).toISOString().replace('T',' ').slice(0,19)+' UTC';}
function v31Bytes(n){const x=Number(n)||0;if(x<1024)return `${x} B`;if(x<1048576)return `${(x/1024).toFixed(1)} KB`;if(x<1073741824)return `${(x/1048576).toFixed(1)} MB`;return `${(x/1073741824).toFixed(2)} GB`;}

async function v31PeekMarketFile(file){
  const head=await file.slice(0,65536).text(),lines=head.replace(/^\uFEFF/,'').split(/\r?\n/).filter(x=>x.trim());
  for(const line of lines){const p=line.split(';').map(x=>x.trim()),format=v31DetectNtFormat(p);if(format)return {format,sample:line};}
  return {format:'',sample:''};
}
async function v31SelectMarketFile(){
  const input=document.createElement('input');input.type='file';input.accept='.txt,text/plain';input.style.display='none';document.body.appendChild(input);
  input.onchange=async()=>{const f=input.files?.[0];input.remove();if(f)await v31PrepareMarketImport(f);};input.click();
}
async function v31PrepareMarketImport(file){
  try{
    const inferred=v31InferFile(file),peek=await v31PeekMarketFile(file);if(!peek.format)return alert('No reconozco el formato. V31 admite exportaciones NinjaTrader Tick, Tick Replay, Minute y Day separadas por punto y coma.');
    v31PendingMarketFile=file;
    const contracts=uniqueSorted(currentOps().map(o=>String(o.contract||'').trim()).filter(Boolean));
    const preferred=contracts.find(x=>v31NormContract(x)===v31NormContract(inferred.sourceContract))||contracts.find(x=>v31RootSymbol(x)===v31RootSymbol(inferred.sourceContract))||inferred.sourceContract;
    const body=`<form onsubmit="return false"><div class="form-section"><h4>Importación NinjaTrader</h4><div class="form-grid"><label class="field span2"><span>Archivo</span><div class="readonly-box">${esc(file.name)} · ${esc(v31Bytes(file.size))}</div></label><label class="field"><span>Formato detectado</span><div class="readonly-box">${esc(v31FormatLabel(peek.format))}</div></label><label class="field"><span>Tipo de precio</span><div class="readonly-box">${esc(peek.format==='tickReplay'?'Last + Bid/Ask':inferred.dataType)}</div></label><label class="field span2"><span>Asociar a contrato / vencimiento de Trading Research</span><input id="v31-md-contract" class="input" list="v31-contracts" value="${esc(preferred)}" placeholder="Ej. MCL 09-26 o MCL"><datalist id="v31-contracts">${contracts.map(x=>`<option value="${esc(x)}"></option>`).join('')}</datalist><small>El nombre del archivo sugiere: ${esc(inferred.sourceContract||'—')}. Puedes mapearlo a cómo escribes el contrato en tus operaciones.</small></label></div><div class="notice" style="margin-top:12px"><strong>Zona horaria:</strong> las exportaciones históricas de NinjaTrader se interpretan como UTC. Los datos brutos se guardarán solo en IndexedDB de este navegador; no entran en Supabase ni en Conflict Guard.</div></div></form>`;
    document.body.insertAdjacentHTML('beforeend',modalShell('Importar Market Data',body,`<button class="btn" onclick="v31PendingMarketFile=null;closeModal()">Cancelar</button><button class="btn primary" onclick="v31ConfirmMarketImport()">Importar histórico</button>`));
  }catch(e){alert('No se pudo preparar el archivo: '+e.message);}
}
async function v31ConfirmMarketImport(){
  const file=v31PendingMarketFile;if(!file)return alert('Selecciona de nuevo el archivo.');
  const contractKey=document.getElementById('v31-md-contract')?.value?.trim();if(!contractKey)return alert('Indica el contrato o vencimiento al que pertenece este histórico.');
  closeModal();currentView='market';v31MarketCache.importStatus={name:file.name,text:'Preparando importación…',rows:0};render();
  try{await v31ImportMarketFile(file,contractKey);v31PendingMarketFile=null;await v31RefreshMarketCache(false);v31MarketCache.importStatus=null;render();alert('Market Data importado correctamente. Ahora puedes validar qué operaciones tienen recorrido reconstruible.');}
  catch(e){v31MarketCache.importStatus=null;render();alert('No se pudo importar el histórico: '+e.message);}
}
function v31UpdateImportStatus(text,rows){v31MarketCache.importStatus={...(v31MarketCache.importStatus||{}),text,rows};const el=document.getElementById('v31-import-status');if(el)el.innerHTML=`<strong>${esc(text)}</strong><span>${Number(rows||0).toLocaleString('es-ES')} registros procesados</span>`;}
async function v31ImportMarketFile(file,contractKey){
  const inferred=v31InferFile(file),peek=await v31PeekMarketFile(file),datasetId=uid('MD'),format=peek.format,dataType=format==='tickReplay'?'Replay':inferred.dataType;
  if(!format)throw new Error('Formato NinjaTrader no reconocido.');
  const meta={id:datasetId,fileName:file.name,fileSize:file.size,sourceContract:inferred.sourceContract,contractKey:String(contractKey).trim(),dataType,format,timezone:'UTC',importedAt:new Date().toISOString(),startMs:null,endMs:null,rowCount:0,chunkCount:0,invalidLines:0,status:'importing'};
  await v31DatasetPut(meta);
  const decoder=new TextDecoder('utf-8'),reader=file.stream().getReader();let carry='',rows=[],seq=0,total=0,invalid=0,start=Infinity,end=-Infinity,detected=format;
  const flush=async()=>{if(!rows.length)return;const chunkRows=rows;rows=[];await v31ChunkPut({id:`${datasetId}:${String(seq).padStart(6,'0')}`,datasetId,seq,startMs:chunkRows[0][0],endMs:chunkRows[chunkRows.length-1][0],rows:chunkRows});seq++;v31UpdateImportStatus(`Importando ${v31FormatLabel(detected)}…`,total);};
  try{
    while(true){const {value,done}=await reader.read();if(done)break;carry+=decoder.decode(value,{stream:true});const lines=carry.split(/\r?\n/);carry=lines.pop()||'';for(const line of lines){if(!line.trim())continue;const parsed=v31ParseNtLine(line,detected);if(!parsed){invalid++;continue;}const r=parsed.row;rows.push(r);total++;if(r[0]<start)start=r[0];if(r[0]>end)end=r[0];if(rows.length>=20000)await flush();}}
    carry+=decoder.decode();if(carry.trim()){const parsed=v31ParseNtLine(carry,detected);if(parsed){rows.push(parsed.row);total++;start=Math.min(start,parsed.row[0]);end=Math.max(end,parsed.row[0]);}else invalid++;}
    await flush();if(!total)throw new Error('El archivo no contiene registros válidos.');
    Object.assign(meta,{startMs:start,endMs:end,rowCount:total,chunkCount:seq,invalidLines:invalid,status:'ready'});await v31DatasetPut(meta);v31UpdateImportStatus('Finalizando índice local…',total);
  }catch(e){await v31DeleteDatasetSilent(datasetId);throw e;}
}

function v31ParseRawLocalDateTime(v){
  const s=String(v||'').trim(),m=s.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})\s+(\d{1,2}):(\d{2})(?::(\d{2}))?/);if(!m)return null;
  const d=new Date(+m[3],+m[2]-1,+m[1],+m[4],+m[5],+(m[6]||0),0);return d.getTime();
}
function v31OpTimeInfo(o,key){
  const rawCols=o?.raw?.columns||{},rawValue=key==='entry'?rawCols.EntryDateTime:rawCols.ExitDateTime,rawMs=v31ParseRawLocalDateTime(rawValue);
  if(Number.isFinite(rawMs))return {ms:rawMs,precision:/\d{1,2}:\d{2}:\d{2}/.test(String(rawValue))?'second':'minute',source:'Ankora original'};
  const v=key==='entry'?o?.entryDate:o?.exitDate,ms=new Date(v||'').getTime(),precision=/T\d{2}:\d{2}:\d{2}/.test(String(v||''))?'second':'minute';
  return {ms:Number.isFinite(ms)?ms:null,precision,source:'operación'};
}
function v31DatasetUsableForExcursion(d){return d&&['tick','tickReplay','minute'].includes(d.format)&&['Last','Replay','Unknown'].includes(d.dataType);}
function v31ContractMatchScore(op,d){
  const a=v31NormContract(op?.contract),b=v31NormContract(d?.contractKey);if(!a||!b)return 0;if(a===b)return 3;return v31RootSymbol(a)&&v31RootSymbol(a)===v31RootSymbol(b)?1:0;
}
function v31FindDatasetForOp(o,datasets=v31MarketCache.datasets){
  const en=v31OpTimeInfo(o,'entry'),ex=v31OpTimeInfo(o,'exit');if(!Number.isFinite(en.ms)||!Number.isFinite(ex.ms)||ex.ms<en.ms)return null;
  const candidates=(datasets||[]).filter(d=>d.status==='ready'&&v31DatasetUsableForExcursion(d)&&v31ContractMatchScore(o,d)>0&&Number(d.startMs)<=en.ms&&Number(d.endMs)>=ex.ms);
  if(!candidates.length)return null;
  return candidates.sort((a,b)=>{const fa=(a.format==='tickReplay'||a.format==='tick'?2:a.format==='minute'?1:0),fb=(b.format==='tickReplay'||b.format==='tick'?2:b.format==='minute'?1:0);return (v31ContractMatchScore(o,b)-v31ContractMatchScore(o,a))||(fb-fa)||Number(b.importedAt)-Number(a.importedAt);})[0];
}
function v31OpMarketStatus(o){
  const en=v31OpTimeInfo(o,'entry'),ex=v31OpTimeInfo(o,'exit'),d=v31FindDatasetForOp(o),tickSize=Number(o?.instrumentSnapshot?.tickSize||dqInstrumentForOp?.(o)?.tickSize||0),entry=Number(o?.entryPrice);
  if(!Number.isFinite(en.ms)||!Number.isFinite(ex.ms))return {key:'time',label:'Faltan timestamps',detail:'Entrada/salida incompletas'};
  if(!d)return {key:'nodata',label:'Sin cobertura',detail:'No hay histórico compatible que cubra entrada → salida'};
  if(!(entry>0))return {key:'entry',label:'Datos presentes',detail:'Falta precio de entrada',dataset:d};
  if(!(tickSize>0))return {key:'ticksize',label:'Datos presentes',detail:'Falta tick size del instrumento',dataset:d};
  const timeExact=en.precision==='second'&&ex.precision==='second',dataExact=['tick','tickReplay'].includes(d.format),exact=timeExact&&dataExact;
  return {key:exact?'exact':'approx',label:exact?'Reconstruible exacto':'Reconstruible con cautela',detail:!dataExact?'Resolución 1 minuto: extremos intrabar sin secuencia exacta':'Market Data tick, pero la operación solo conserva hora al minuto',dataset:d,timeExact,dataExact};
}
function v31CoverageSummary(){
  const ops=currentOps(),statuses=ops.map(v31OpMarketStatus),covered=statuses.filter(s=>s.dataset).length,ready=statuses.filter(s=>['exact','approx'].includes(s.key)).length,exact=statuses.filter(s=>s.key==='exact').length;
  return {total:ops.length,covered,ready,exact,pct:ops.length?covered/ops.length*100:0,readyPct:ops.length?ready/ops.length*100:0,exactPct:ops.length?exact/ops.length*100:0};
}
function v31StatusBadge(s){const cls=s.key==='exact'?'win':s.key==='approx'||s.key==='entry'||s.key==='ticksize'?'warn':'';return `<span class="badge ${cls}">${esc(s.label)}</span>`;}
function v31MarketDatasetTable(){
  const ds=v31MarketCache.datasets;if(!ds.length)return '<div class="empty">Todavía no hay históricos locales. Exporta desde NinjaTrader y pulsa “Importar histórico”.</div>';
  return `<div class="table-wrap"><table class="table market-data-table"><thead><tr><th>Contrato TR</th><th>Origen</th><th>Formato</th><th>Tipo</th><th>Rango UTC</th><th>Registros</th><th>Archivo</th><th></th></tr></thead><tbody>${ds.map(d=>`<tr><td><strong>${esc(d.contractKey)}</strong></td><td>${esc(d.sourceContract||'—')}</td><td>${esc(v31FormatLabel(d.format))}</td><td>${esc(v31DataTypeLabel(d.dataType))}</td><td><small>${esc(v31FormatUtc(d.startMs))}<br>${esc(v31FormatUtc(d.endMs))}</small></td><td>${Number(d.rowCount||0).toLocaleString('es-ES')}<br><small>${Number(d.invalidLines||0)?`${d.invalidLines} línea(s) ignorada(s)`:''}</small></td><td><small>${esc(d.fileName)}<br>${esc(v31Bytes(d.fileSize))}</small></td><td><div class="table-actions"><button class="btn tiny" onclick="v31EditDataset('${esc(d.id)}')">Mapear</button><button class="btn tiny danger" onclick="v31DeleteDataset('${esc(d.id)}')">Eliminar</button></div></td></tr>`).join('')}</tbody></table></div>`;
}
function v31MarketOpsTable(){
  const rows=[...currentOps()].sort((a,b)=>(v31OpTimeInfo(b,'entry').ms||0)-(v31OpTimeInfo(a,'entry').ms||0)).slice(0,80);
  if(!rows.length)return '<div class="empty">No hay operaciones en este Trading Plan.</div>';
  return `<div class="table-wrap"><table class="table market-op-table"><thead><tr><th>Entrada</th><th>Contrato</th><th>Precio entrada</th><th>Market Data</th><th>Precisión</th><th></th></tr></thead><tbody>${rows.map(o=>{const s=v31OpMarketStatus(o),d=s.dataset;return `<tr><td>${esc(fmtDate(o.entryDate))}</td><td><strong>${esc(o.contract||o.instrumentSnapshot?.symbol||'—')}</strong></td><td>${Number(o.entryPrice)>0?Number(o.entryPrice):'—'}</td><td>${d?`${esc(d.contractKey)} · ${esc(v31FormatLabel(d.format))}`:'—'}</td><td>${v31StatusBadge(s)}<br><small>${esc(s.detail||'')}</small></td><td>${d&&['exact','approx'].includes(s.key)?`<button class="btn tiny primary" onclick="v31AnalyzeOperation('${esc(o.id)}')">Calcular recorrido</button>`:''}</td></tr>`;}).join('')}</tbody></table></div><div class="help">Se muestran hasta 80 operaciones recientes. La cobertura se calcula sobre todo el Trading Plan.</div>`;
}
function v31MarketDataView(){
  if(!v31MarketCache.loaded&&!v31MarketCache.loading)setTimeout(()=>v31RefreshMarketCache(true),0);
  const c=v31CoverageSummary(),status=v31MarketCache.importStatus;
  const controls='<button class="btn primary" onclick="v31SelectMarketFile()">Importar histórico NinjaTrader</button>';
  return `${pageHead('Market Data Foundation','Capa local de datos históricos para reconstruir el recorrido real entrada → salida y preparar simulaciones de gestión sin adulterar MAE/MFE.',controls)}${activePlanBanner()}<div class="notice market-local-notice"><strong>Local y pesado por diseño:</strong> los ticks/minutos se guardan únicamente en IndexedDB de este navegador. No se incluyen en la sincronización Supabase, snapshots de Conflict Guard ni backups del workspace. V31 interpreta exportaciones NinjaTrader como UTC.</div>${status?`<section class="card market-import-progress"><div id="v31-import-status"><strong>${esc(status.text||'Importando…')}</strong><span>${Number(status.rows||0).toLocaleString('es-ES')} registros procesados</span></div></section>`:''}<div class="review-kpis market-kpis">${kpi('Operaciones',c.total,'plan activo')}${kpi('Con Market Data',c.covered,`${c.pct.toFixed(0)}% cubiertas`)}${kpi('Reconstruibles',c.ready,`${c.readyPct.toFixed(0)}% con precio/tick size`)}${kpi('Precisión tick + segundos',c.exact,`${c.exactPct.toFixed(0)}% exactas`)}</div><section class="card panel"><div class="panel-title"><div><h3>Biblioteca local de históricos</h3><small>NinjaTrader Tick / Tick Replay / Minute / Day. Para MAE/MFE automático V31 usa Last/Tick Replay y permite Minute solo como estimación.</small></div></div>${v31MarketDatasetTable()}</section><section class="card panel market-validation-panel"><div class="panel-title"><div><h3>Validación operación × recorrido</h3><small>Primero comprobamos que el histórico correcto cubre exactamente la vida real del trade; después calculamos excursiones.</small></div></div>${v31MarketOpsTable()}</section><div class="notice"><strong>Regla V31:</strong> los datos Tick pueden resolver el orden del precio, pero si la operación solo conserva timestamp al minuto, el límite de entrada/salida sigue siendo aproximado. Para aplicar MAE/MFE automáticamente exigimos Tick/Tick Replay + timestamps de entrada y salida con segundos. Los datos de 1 minuto sirven para diagnóstico, no para sobrescribir MAE/MFE medidos.</div>`;
}

async function v31EditDataset(id){
  const d=await v31DatasetGet(id);if(!d)return;
  const contracts=uniqueSorted(currentOps().map(o=>String(o.contract||'').trim()).filter(Boolean));
  const body=`<form onsubmit="return false"><div class="form-section"><h4>Mapeo local</h4><div class="form-grid"><label class="field span2"><span>Archivo</span><div class="readonly-box">${esc(d.fileName)}</div></label><label class="field span2"><span>Contrato / vencimiento en Trading Research</span><input id="v31-map-contract" class="input" list="v31-map-contracts" value="${esc(d.contractKey)}"><datalist id="v31-map-contracts">${contracts.map(x=>`<option value="${esc(x)}"></option>`).join('')}</datalist></label></div><div class="notice" style="margin-top:12px">Cambiar este mapeo no altera el archivo ni las operaciones; solo controla qué trades pueden asociarse a este histórico.</div></div></form>`;
  document.body.insertAdjacentHTML('beforeend',modalShell('Mapear Market Data',body,`<button class="btn" onclick="closeModal()">Cancelar</button><button class="btn primary" onclick="v31SaveDatasetMapping('${esc(id)}')">Guardar mapeo</button>`));
}
async function v31SaveDatasetMapping(id){const d=await v31DatasetGet(id),v=document.getElementById('v31-map-contract')?.value?.trim();if(!d||!v)return alert('Indica un contrato.');d.contractKey=v;d.updatedAt=new Date().toISOString();await v31DatasetPut(d);closeModal();await v31RefreshMarketCache(true);}

async function v31RowsForRange(dataset,startMs,endMs){
  const db=await v31MarketDb(),rows=[];
  await new Promise((resolve,reject)=>{
    const tx=db.transaction(V31_MARKET_CHUNKS,'readonly'),req=tx.objectStore(V31_MARKET_CHUNKS).index('datasetId').openCursor(IDBKeyRange.only(dataset.id));
    req.onsuccess=()=>{const c=req.result;if(!c)return;const v=c.value;if(Number(v.endMs)>=startMs&&Number(v.startMs)<=endMs+(dataset.format==='minute'?60000:0)){for(const r of v.rows||[]){const t=Number(r[0]);if(dataset.format==='minute'){if(t>startMs&&t<=endMs+60000)rows.push(r);}else if(t>=startMs&&t<=endMs)rows.push(r);}}c.continue();};
    tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error);
  });
  rows.sort((a,b)=>a[0]-b[0]);return rows;
}
function v31TickSizeForOp(o){return Number(o?.instrumentSnapshot?.tickSize||((state.settings?.instruments||[]).find(i=>i.id===o?.instrumentId||i.symbol===v31RootSymbol(o?.contract))?.tickSize)||0);}
async function v31ComputeOperationPath(o,d){
  const en=v31OpTimeInfo(o,'entry'),ex=v31OpTimeInfo(o,'exit'),entry=Number(o.entryPrice),tickSize=v31TickSizeForOp(o);if(!(entry>0))throw new Error('La operación no tiene precio de entrada.');if(!(tickSize>0))throw new Error('El instrumento no tiene tick size válido.');
  const rows=await v31RowsForRange(d,en.ms,ex.ms);if(!rows.length)throw new Error('El dataset está dentro del rango global, pero no hay registros entre entrada y salida. Revisa timestamps/huecos.');
  let hi=-Infinity,lo=Infinity,first=null,last=null;
  if(d.format==='tick'||d.format==='tickReplay'){for(const r of rows){const px=Number(r[1]);if(!Number.isFinite(px))continue;hi=Math.max(hi,px);lo=Math.min(lo,px);if(first===null)first=px;last=px;}}
  else if(d.format==='minute'){for(const r of rows){hi=Math.max(hi,Number(r[2]));lo=Math.min(lo,Number(r[3]));if(first===null)first=Number(r[1]);last=Number(r[4]);}}
  if(!Number.isFinite(hi)||!Number.isFinite(lo))throw new Error('No se pudieron reconstruir máximos/mínimos.');
  const long=String(o.direction).toUpperCase()!=='SHORT',mfePrice=Math.max(0,long?hi-entry:entry-lo),maePrice=Math.max(0,long?entry-lo:hi-entry),mfeTicks=mfePrice/tickSize,maeTicks=maePrice/tickSize;
  const exactData=['tick','tickReplay'].includes(d.format),exactTime=en.precision==='second'&&ex.precision==='second',exact=exactData&&exactTime;
  return {dataset:d,rowCount:rows.length,entryMs:en.ms,exitMs:ex.ms,entryPrecision:en.precision,exitPrecision:ex.precision,timeSource:`${en.source} / ${ex.source}`,entry,first,last,high:hi,low:lo,tickSize,mfeTicks,maeTicks,mfeR:v301ExcursionRFromTicks(mfeTicks,o),maeR:v301ExcursionRFromTicks(maeTicks,o),exact,exactData,exactTime};
}
async function v31AnalyzeOperation(id){
  const o=state.operations.find(x=>x.id===id);if(!o)return;const d=v31FindDatasetForOp(o);if(!d)return alert('No hay Market Data compatible para esta operación.');
  document.body.insertAdjacentHTML('beforeend',modalShell('Reconstrucción Market Data','<div class="empty">Calculando recorrido…</div>','<button class="btn" onclick="closeModal()">Cerrar</button>'));
  try{
    const r=await v31ComputeOperationPath(o,d),precision=r.exact?'Exactitud operativa V31':'Estimación / validar manualmente';
    const body=`<div class="market-analysis"><div class="market-analysis-head ${r.exact?'exact':'approx'}"><strong>${esc(precision)}</strong><span>${esc(v31FormatLabel(d.format))} · ${esc(d.contractKey)}</span></div><div class="trade-detail-kpis market-analysis-kpis"><div><span>MFE reconstruido</span><strong>${r.mfeTicks.toFixed(1)}t · ${r.mfeR.toFixed(2)}R</strong></div><div><span>MAE reconstruido</span><strong>${r.maeTicks.toFixed(1)}t · ${r.maeR.toFixed(2)}R</strong></div><div><span>Máximo / mínimo</span><strong>${r.high} / ${r.low}</strong></div><div><span>Registros usados</span><strong>${r.rowCount.toLocaleString('es-ES')}</strong></div></div><div class="market-analysis-grid"><div><span>Entrada registrada</span><strong>${r.entry}</strong><small>${esc(new Date(r.entryMs).toLocaleString())}</small></div><div><span>Salida registrada</span><strong>${esc(fmtDate(o.exitDate))}</strong><small>${esc(new Date(r.exitMs).toLocaleString())}</small></div><div><span>Resolución Market Data</span><strong>${esc(v31FormatLabel(d.format))}</strong><small>${r.exactData?'secuencia tick disponible':'OHLC: orden intrabar no conocido'}</small></div><div><span>Precisión timestamps trade</span><strong>${r.entryPrecision} / ${r.exitPrecision}</strong><small>${esc(r.timeSource)}</small></div></div>${r.exact?'<div class="notice good-note"><strong>Apto para aplicar:</strong> Market Data tick y límites de la operación con segundos. El cálculo usa exclusivamente entrada → salida final real.</div>':'<div class="notice"><strong>No sobrescribir automáticamente:</strong> falta resolución tick o los timestamps del trade solo llegan al minuto. El cálculo sirve para contrastar, pero los extremos de la primera/última fracción pueden quedar sesgados.</div>'}</div>`;
    closeModal();document.body.insertAdjacentHTML('beforeend',modalShell('Reconstrucción Market Data',body,`<button class="btn" onclick="closeModal()">Cerrar</button>${r.exact?`<button class="btn primary" onclick="v31ApplyExcursions('${esc(o.id)}','${esc(d.id)}',${r.mfeTicks},${r.maeTicks})">Aplicar MAE/MFE medidos</button>`:''}`));
  }catch(e){closeModal();alert('No se pudo reconstruir la operación: '+e.message);}
}
async function v31ApplyExcursions(opId,datasetId,mfeTicks,maeTicks){
  const o=state.operations.find(x=>x.id===opId),d=await v31DatasetGet(datasetId);if(!o||!d)return;if(!confirm(`Aplicar a la operación MFE ${Number(mfeTicks).toFixed(1)} ticks y MAE ${Number(maeTicks).toFixed(1)} ticks como datos medidos desde Market Data?`))return;
  o.mfeTicks=Math.max(0,Number(mfeTicks)||0);o.maeTicks=Math.max(0,Number(maeTicks)||0);o.mfe=v301ExcursionRFromTicks(o.mfeTicks,o);o.mae=v301ExcursionRFromTicks(o.maeTicks,o);o.dataQuality={...(o.dataQuality||{}),mfeStatus:'measured',maeStatus:'measured',reviewedAt:new Date().toISOString()};o.excursionMeta={definition:V302_EXCURSION_DEFINITION,unit:'ticks',window:'entry_to_final_exit',source:'market_data',datasetId:d.id,datasetFile:d.fileName,resolution:d.format,updatedAt:new Date().toISOString()};o.updatedAt=new Date().toISOString();persist();closeModal();v30TrackNow?.('manual');render();
}

/* Nueva captura de timestamps manuales con segundos para que el futuro motor tick pueda fijar los límites con precisión. */
const operationFormV31Base=operationForm;
operationForm=function(o,r,p){let html=operationFormV31Base(o,r,p);html=html.replace(/(<input id="f-entryDate"[^>]*type="datetime-local"[^>]*)(>)/,'$1 step="1"$2').replace(/(<input id="f-exitDate"[^>]*type="datetime-local"[^>]*)(>)/,'$1 step="1"$2');return html;};

/* Navegación / render sin tocar el payload cloud existente. */
const shellV31Base=shell;
shell=function(){
  let html=shellV31Base(),qualityButton=navBtn('quality','◇','Calidad datos');
  if(html.includes(qualityButton))html=html.replace(qualityButton,qualityButton+navBtn('market','⌗','Market Data'));
  else{const labButton=navBtn('lab','⌁','Laboratorio');html=html.replace(labButton,navBtn('market','⌗','Market Data')+labButton);}
  return html;
};
const renderV31Base=render;
render=function(){
  if(currentView!=='market')return renderV31Base();
  v30EnsureBaselineLocal();document.getElementById('app').innerHTML=shell();const view=document.getElementById('view');view.innerHTML=v31MarketDataView();setTimeout(hydrateImageElements,0);
};
v30ModeCard=function(){return `<div class="side-bottom"><div class="mini-card mode-card ${v30Ui.modeExpanded?'expanded':''}"><button class="mode-card-toggle" onclick="toggleModeCard()"><span><small>Modo actual</small><strong>V31</strong></span><b class="mode-card-arrow">${v30Ui.modeExpanded?'▾':'▴'}</b></button><div class="mode-card-detail"><div class="mini-value">${esc(V31_APP_LABEL)}</div><div class="help">Motor cloud V9.2 Conflict Guard intacto. Market Data local en IndexedDB + importador NinjaTrader UTC + cobertura operación×histórico + reconstrucción entrada→salida + MAE/MFE automáticos solo con precisión suficiente + Data Quality semántico + Change Tracking + resto de research.</div></div></div></div>`;};

if(typeof CONTEXT_HELP!=='undefined')CONTEXT_HELP.push({id:'marketdata',terms:['market data','datos históricos','ninjatrader export','tick data','recorrido de mercado'],title:'Market Data Foundation',summary:'Biblioteca local de históricos Tick/Minute para reconstruir el recorrido de cada operación.',body:'V31 importa el formato histórico de NinjaTrader, cuyos exports se expresan en UTC. Los archivos grandes permanecen en IndexedDB y no se incluyen en el payload Supabase. Una operación se considera reconstruible cuando un histórico compatible cubre entrada→salida y existen precio de entrada y tick size.',use:'Es la base para automatizar MAE/MFE intratrade y, más adelante, simular stops/targets alternativos sin confundir el recorrido posterior con MAE/MFE estándar.'});
Object.assign(window,{v31SelectMarketFile,v31PrepareMarketImport,v31ConfirmMarketImport,v31DeleteDataset,v31EditDataset,v31SaveDatasetMapping,v31AnalyzeOperation,v31ApplyExcursions,v31RefreshMarketCache});
