...
/* ===== V31.6 PATCH · Execution Reconciliation ===== */
...
const v316Ui={...(window.v316Ui||{}),tab:'reconcile'};
...
async function v316SetExecEnvironment(value){const set=await v314StoreGet('execSets',v314MarketUi.activeExecId);if(!set)return;set.executionEnvironment=value;await v314StorePut('execSets',set);await v314RefreshMarketDataState();render();}
function v316EnvForSet(set){return set?.executionEnvironment||'replay';}
function v316CandidateOpsCount(){return currentOps().filter(v316EligibleOp).length;}
...
function v316ReconcileView(set,meta){const eligible=currentOps().filter(v316EligibleOp),linked=currentOps().filter(o=>o.executionEvidence?.linkedAt);return `<section class="card panel reconcile-hero"><div class="panel-title"><div><h3>Execution Reconciliation</h3><small>Une contexto cualitativo del Journal con evidencia objetiva de ejecución NinjaTrader.</small></div></div><div class="reconcile-kpis"><div><span>Operaciones elegibles</span><strong>${eligible.length}</strong><small>Pendiente / Replay / Sim / Live</small></div><div><span>Vinculadas en el plan</span><strong>${linked.length}</strong><small>con evidencia NT</small></div><div><span>Backtest Ankora</span><strong>${currentOps().filter(o=>o.raw?.source==='ankora').length}</strong><small>excluidas automáticamente</small></div><label><span>Entorno del Grid activo</span><select class="select" onchange="v316SetExecEnvironment(this.value)">${[['replay','Replay'],['sim','Sim'],['live','Live']].map(([v,l])=>`<option value="${v}" ${v316EnvForSet(set)===v?'selected':''}>${l}</option>`).join('')}</select></label></div><div class="notice"><strong>Regla de propiedad:</strong> NinjaTrader puede completar o corregir campos de ejecución (hora/precio/cantidad/salida/MFE/MAE), pero nunca sobrescribe Setup, VD, NR, hipótesis, checklist, errores, diario emocional, notas o capturas. Una operación Ankora nunca se convierte en ejecución por coincidencia temporal.</div></section><section class="card panel"><div class="panel-title"><div><h3>Candidatos del Grid</h3><small>${set?esc(set.sourceFile):'Sin Grid seleccionado'} · la vinculación requiere confirmación explícita.</small></div></div>${v316ReconciliationTable(set,meta)}</section>`;}
