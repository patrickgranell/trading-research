...
const V3110_APP_LABEL='V31.10.1 · Best Exit / What-if · resting limits';
...
function v3110ObservedEvidence(result,series){
  ...
  const actual=Number(result?.realizedTicks)||0,end=points[points.length-1],positive=points.filter(p=>p.exitPnlTicks>0).length;
  return {points,best,worst,end,actual,gap:best.exitPnlTicks-actual,capturePct:best.exitPnlTicks>0?actual/best.exitPnlTicks*100:null,positivePct:points.length?positive/points.length*100:0,timeToBest:Math.max(0,best.ms-series.startMs),afterBest:Math.max(0,series.endMs-best.ms)};
}
...
function v3110ExitChart(result,series,evidence,target,trail){
  ...
  const vals=pts.map(p=>p.exitPnlTicks).concat([actual,0,target?.targetTicks||0]);let ymin=Math.min(...vals),ymax=Math.max(...vals),pad=Math.max(1,(ymax-ymin)*.12);ymin-=pad;ymax+=pad;if(Math.abs(ymax-ymin)<1e-9){ymin-=1;ymax+=1;}
  ...
  const targetLine=target?.targetTicks>=ymin&&target.targetTicks<=ymax?`<line x1="${L}" y1="${y(target.targetTicks)}" x2="${W-R}" y2="${y(target.targetTicks)}" class="be-target-line"/><text x="${W-R-5}" y="${y(target.targetTicks)-5}" text-anchor="end" class="be-target-label">TP +${v315Nice(target.targetTicks,1)}t</text>`:'';
  const best=evidence.best,bestMark=`<circle cx="${x(best)}" cy="${y(best.exitPnlTicks)}" r="6" class="be-best"/><text x="${x(best)+8}" y="${y(best.exitPnlTicks)-8}" class="be-best-label">máx marketable ${v314SignedTicks(best.exitPnlTicks)}</text>`;
...
function v3110BestExitPanel(set,meta){
  ...
  const gapText=e.gap>=0?`${v314SignedTicks(e.gap)} cedidos frente al mejor cierre marketable`:`fill real ${v314SignedTicks(-e.gap)} mejor que el mejor cierre marketable observado`;
  const effValue=e.gap<0?v314SignedTicks(-e.gap):v3110FmtPct(e.capturePct),effClass=e.gap<0?'positive':'',effText=e.gap<0?'ventaja del fill pasivo frente al benchmark Bid/Ask':'porcentaje del máximo marketable capturado';
  const kpis=`<div class="be-kpis"><div><span>Fill real</span><strong class="${v3110ResultClass(actual)}">${v314SignedTicks(actual)}</strong><small>${v3110FmtPrice(r.exitPrice,s)} ejecutado</small></div><div><span>Máximo marketable</span><strong class="${v3110ResultClass(e.best.exitPnlTicks)}">${v314SignedTicks(e.best.exitPnlTicks)}</strong><small>${v3110FmtPrice(e.best.exitQuote,s)} · ${v3110When(e.best.ms,s)}</small></div><div><span>Gap vs marketable</span><strong class="${e.gap>0?'negative':e.gap<0?'positive':''}">${v314SignedTicks(e.gap)}</strong><small>${esc(gapText)}</small></div><div><span>Eficiencia de salida</span><strong class="${effClass}">${effValue}</strong><small>${esc(effText)}</small></div><div><span>Peor quote</span><strong class="${v3110ResultClass(e.worst.exitPnlTicks)}">${v314SignedTicks(e.worst.exitPnlTicks)}</strong><small>${v3110FmtPrice(e.worst.exitQuote,s)}</small></div><div><span>Tiempo en positivo</span><strong>${v3110FmtPct(e.positivePct)}</strong><small>${v315Duration(e.timeToBest)} hasta máximo · ${v315Duration(e.afterBest)} después</small></div></div>`;
...
if(typeof CONTEXT_HELP!=='undefined')CONTEXT_HELP.push({id:'bestexitwhatif',terms:['best exit','what-if salida','what if salida','giveback','salida por tiempo','tp fijo intratrade'],title:'Best Exit / What-if',summary:'Compara el fill real con oportunidades de salida observadas dentro de la misma ventana intratrade.',body:'El máximo cotizable usa Bid para LONG y Ask para SHORT y funciona solo como benchmark retrospectivo. Los escenarios por tiempo y giveback usan el quote marketable contemporáneo sin slippage/latencia; el TP fijo supone una orden límite ya resting y exige cruce del quote. Nunca usa datos posteriores a la salida real.',use:'Úsalo para formular hipótesis de gestión de salida que luego puedas probar en una muestra amplia; no conviertas el mejor punto hindsight de una operación en una regla.'});

v30ModeCard=function(){return `<div class="side-bottom"><div class="mini-card mode-card ${v30Ui.modeExpanded?'expanded':''}"><button class="mode-card-toggle" onclick="toggleModeCard()"><span><small>Modo actual</small><strong>V31.10.1</strong></span><b class="mode-card-arrow">${v30Ui.modeExpanded?'▾':'▴'}</b></button><div class="mode-card-detail"><div class="mini-value">${esc(V3110_APP_LABEL)}</div><div class="help">Market Data añade Best Exit / What-if sobre la serie intratrade ya calibrada. El recorrido sigue siendo Last; los benchmarks de salida usan Bid/Ask según el lado de cierre, distinguen evidencia descriptiva de contrafactual y jamás miran después del fill real.</div></div></div></div>`;};
