...
const V3110_APP_LABEL='V31.10.2 · Best Exit / What-if · Δ fill vs marketable';
...
function v3110BestExitPanel(set,meta){
  ...
  const effValue=fillVsMarketable>0?v314SignedTicks(fillVsMarketable):v3110FmtPct(e.capturePct),effClass=fillVsMarketable>0?'positive':'',effText=fillVsMarketable>0?'ventaja del fill pasivo frente al benchmark Bid/Ask':'porcentaje del máximo marketable capturado';
  const kpis=`<div class="be-kpis"><div><span>Fill real</span><strong class="${v3110ResultClass(actual)}">${v314SignedTicks(actual)}</strong><small>${v3110FmtPrice(r.exitPrice,s)} ejecutado</small></div><div><span>Máximo marketable</span><strong class="${v3110ResultClass(e.best.exitPnlTicks)}">${v314SignedTicks(e.best.exitPnlTicks)}</strong><small>${v3110FmtPrice(e.best.exitQuote,s)} · ${v3110When(e.best.ms,s)}</small></div><div><span>Δ fill vs marketable</span><strong class="${v3110ResultClass(fillVsMarketable)}">${v314SignedTicks(fillVsMarketable)}</strong><small>${esc(gapText)}</small></div><div><span>Eficiencia de salida</span><strong class="${effClass}">${effValue}</strong><small>${esc(effText)}</small></div><div><span>Peor quote</span><strong class="${v3110ResultClass(e.worst.exitPnlTicks)}">${v314SignedTicks(e.worst.exitPnlTicks)}</strong><small>${v3110FmtPrice(e.worst.exitQuote,s)}</small></div><div><span>Tiempo en positivo</span><strong>${v3110FmtPct(e.positivePct)}</strong><small>${v315Duration(e.timeToBest)} hasta máximo · ${v315Duration(e.afterBest)} después</small></div></div>`;
...
if(typeof CONTEXT_HELP!=='undefined')CONTEXT_HELP.push({id:'bestexitwhatif',terms:['best exit','what-if salida','what if salida','giveback','salida por tiempo','tp fijo intratrade'],title:'Best Exit / What-if',summary:'Compara el fill real con oportunidades de salida observadas dentro de la misma ventana intratrade.',body:'El máximo marketable usa Bid para LONG y Ask para SHORT y funciona solo como benchmark retrospectivo. Los escenarios por tiempo y giveback usan el quote marketable contemporáneo sin slippage/latencia; el TP fijo supone una orden límite ya resting y exige cruce del quote. Nunca usa datos posteriores a la salida real.',use:'Úsalo para formular hipótesis de gestión de salida que luego puedas probar en una muestra amplia; no conviertas el mejor punto hindsight de una operación en una regla.'});

v30ModeCard=function(){return `<div class="side-bottom"><div class="mini-card mode-card ${v30Ui.modeExpanded?'expanded':''}"><button class="mode-card-toggle" onclick="toggleModeCard()"><span><small>Modo actual</small><strong>V31.10.2</strong></span><b class="mode-card-arrow">${v30Ui.modeExpanded?'▾':'▴'}</b></button><div class="mode-card-detail"><div class="mini-value">${esc(V3110_APP_LABEL)}</div><div class="help">Market Data añade Best Exit / What-if sobre la serie intratrade ya calibrada. El recorrido sigue siendo Last; los benchmarks de salida usan Bid/Ask según el lado de cierre, distinguen evidencia descriptiva de contrafactual y jamás miran después del fill real.</div></div></div></div>`;};
