...
const V3110_APP_LABEL='V31.10 · Best Exit / What-if intratrade';
...
function v3110TargetScenario(result,series,evidence,targetTicks=v3110Ui.targetTicks){
  const t=v3110ClampTicks(targetTicks,20),hit=evidence.points.find(p=>p.exitPnlTicks>=t)||null,ts=Number(series.tickSize)||.01,entry=Number(result.entryPrice),dir=String(result.direction||'').toUpperCase(),targetPrice=dir==='LONG'?entry+t*ts:entry-t*ts;
  return {kind:'target',label:`TP fijo +${v315Nice(t,1)}t`,targetTicks:t,targetPrice,hit,resultTicks:hit?t:null,price:hit?targetPrice:null,ms:hit?.ms??null,detail:hit?'límite resting · quote cruzó el nivel':'no alcanzado'};
}
...
function v3110ScenarioRow(s,actual,series){
  const has=Number.isFinite(Number(s?.resultTicks)),delta=has?Number(s.resultTicks)-Number(actual):NaN,status=s?.kind==='target'&&!s.hit?'No alcanzado':s?.kind==='trail'&&!s.exit?(s?.armed?'Armado · sin salida':'No activa'):'Salida hipotética';
  return `<tr><th>${esc(s?.label||'Escenario')}<small>${esc(s?.detail||'')}</small></th><td>${has?esc(v3110When(s.ms,series)):esc(status)}</td><td>${has?esc(v3110FmtPrice(s.price,series)):'—'}</td><td class="${has?v3110ResultClass(s.resultTicks):''}">${has?v314SignedTicks(s.resultTicks):'—'}</td><td class="${Number.isFinite(delta)?v3110ResultClass(delta):''}">${Number.isFinite(delta)?v3110FmtDelta(delta):'—'}</td></tr>`;
}
...
function v3110ExitChart(result,series,evidence,target,trail){
  ...
  const best=evidence.best,bestMark=`<circle cx="${x(best)}" cy="${y(best.exitPnlTicks)}" r="6" class="be-best"/><text x="${x(best)+8}" y="${y(best.exitPnlTicks)-8}" class="be-best-label">máx ${v314SignedTicks(best.exitPnlTicks)}</text>`;
...
function v3110BestExitPanel(set,meta){
  ...
  const gapText=e.gap>=0?`${v314SignedTicks(e.gap)} cedidos desde el máximo`:`${v314SignedTicks(-e.gap)} mejor que el máximo quote observado`;
  const kpis=`<div class="be-kpis"><div><span>Fill real</span><strong class="${v3110ResultClass(actual)}">${v314SignedTicks(actual)}</strong><small>${v3110FmtPrice(r.exitPrice,s)} ejecutado</small></div><div><span>Máximo cotizable</span><strong class="${v3110ResultClass(e.best.exitPnlTicks)}">${v314SignedTicks(e.best.exitPnlTicks)}</strong><small>${v3110FmtPrice(e.best.exitQuote,s)} · ${v3110When(e.best.ms,s)}</small></div><div><span>Gap de salida</span><strong class="${e.gap>0?'negative':e.gap<0?'positive':''}">${v314SignedTicks(e.gap)}</strong><small>${esc(gapText)}</small></div><div><span>Captura del máximo</span><strong>${v3110FmtPct(e.capturePct)}</strong><small>fill real ÷ mejor quote</small></div><div><span>Peor quote</span><strong class="${v3110ResultClass(e.worst.exitPnlTicks)}">${v314SignedTicks(e.worst.exitPnlTicks)}</strong><small>${v3110FmtPrice(e.worst.exitQuote,s)}</small></div><div><span>Tiempo en positivo</span><strong>${v3110FmtPct(e.positivePct)}</strong><small>${v315Duration(e.timeToBest)} hasta máximo · ${v315Duration(e.afterBest)} después</small></div></div>`;
  const scenarios=[...timeScenarios,target,trail];
  const controls=`<div class="be-controls"><div><label><span>TP fijo</span><div class="be-input-unit"><input class="input" type="number" min="1" max="500" step="1" value="${v3110Ui.targetTicks}" onchange="v3110SetTargetTicks(this.value)"><b>ticks</b></div></label><small>Se considera alcanzado solo si el quote de salida (${r.direction==='LONG'?'Bid':'Ask'}) cruza el nivel.</small></div><div><label><span>Activar giveback al llegar a</span><div class="be-input-unit"><input class="input" type="number" min="1" max="500" step="1" value="${v3110Ui.trailTriggerTicks}" onchange="v3110SetTrailTrigger(this.value)"><b>ticks</b></div></label><small>El trailing no existe antes de este trigger.</small></div><div><label><span>Cesión permitida desde máximo</span><div class="be-input-unit"><input class="input" type="number" min="1" max="500" step="1" value="${v3110Ui.trailGivebackTicks}" onchange="v3110SetTrailGiveback(this.value)"><b>ticks</b></div></label><small>Primera caída de este tamaño desde el máximo posterior al trigger.</small></div></div>`;
  const table=`<div class="be-scenario-grid"><section class="be-subcard"><div class="exit-subtitle"><h4>What-if dentro de la ventana real</h4><small>Δ frente al fill realmente ejecutado.</small></div><div class="table-wrap"><table class="table be-table"><thead><tr><th>Escenario</th><th>Cuándo</th><th>Precio</th><th>Resultado</th><th>Δ vs real</th></tr></thead><tbody>${scenarios.map(x=>v3110ScenarioRow(x,actual,s)).join('')}</tbody></table></div></section><section class="be-subcard"><div class="exit-subtitle"><h4>Mapa de oportunidades observadas</h4><small>Primer momento en que el quote de salida alcanzó cada nivel.</small></div><div class="table-wrap"><table class="table be-table"><thead><tr><th>Nivel</th><th>Alcanzado</th><th>Primera vez</th><th>Quote</th></tr></thead><tbody>${v3110TargetMap(e,s)}</tbody></table></div></section></div>`;
  ...
  const method=`<div class="notice be-method"><strong>Qué significa esta pestaña:</strong> el fill real sigue siendo la verdad ejecutada. El “máximo cotizable” es un benchmark retrospectivo, no una estrategia. Las salidas por tiempo y giveback usan el <b>${r.direction==='LONG'?'Bid':'Ask'}</b> contemporáneo como precio marketable de referencia, sin modelar latencia, slippage, cola ni parciales. El TP fijo supone una orden límite ya resting y solo se marca como alcanzado si el quote cruza el nivel. <b>Ningún cálculo mira después de la salida real.</b> Los resultados de esta pestaña son <b>ticks brutos por contrato</b>; si el tamaño no cambia, la comisión round-turn es constante y no altera el Δ frente al fill real.</div>`;
...
v30ModeCard=function(){return `<div class="side-bottom"><div class="mini-card mode-card ${v30Ui.modeExpanded?'expanded':''}"><button class="mode-card-toggle" onclick="toggleModeCard()"><span><small>Modo actual</small><strong>V31.10</strong></span><b class="mode-card-arrow">${v30Ui.modeExpanded?'▾':'▴'}</b></button><div class="mode-card-detail"><div class="mini-value">${esc(V3110_APP_LABEL)}</div><div class="help">Market Data añade Best Exit / What-if sobre la serie intratrade ya calibrada. El recorrido sigue siendo Last; los benchmarks de salida usan Bid/Ask según el lado de cierre, distinguen evidencia descriptiva de contrafactual y jamás miran después del fill real.</div></div></div></div>`;};
