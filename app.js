...
const V3110_APP_LABEL='V31.10.3 · Best Exit / What-if · escala y retención';
...
function v3110BestExitPanel(set,meta){
  ...
  const effValue=fillVsMarketable>0?v314SignedTicks(fillVsMarketable):bestPositive?v3110FmtPct(retentionPct):'—',effClass=fillVsMarketable>0?'positive':actual<0?'negative':'',effText=fillVsMarketable>0?'ventaja del fill pasivo frente al benchmark Bid/Ask':bestPositive?(actual>0?'porcentaje del máximo marketable retenido al cerrar':'se cedió todo el máximo marketable positivo antes del cierre'):'no hubo beneficio marketable positivo que retener';
  const kpis=`<div class="be-kpis"><div><span>Fill real</span><strong class="${v3110ResultClass(actual)}">${v314SignedTicks(actual)}</strong><small>${v3110FmtPrice(r.exitPrice,s)} ejecutado</small></div><div><span>Máximo marketable</span><strong class="${v3110ResultClass(e.best.exitPnlTicks)}">${v314SignedTicks(e.best.exitPnlTicks)}</strong><small>${v3110FmtPrice(e.best.exitQuote,s)} · ${v3110When(e.best.ms,s)}</small></div><div><span>Δ fill vs marketable</span><strong class="${v3110ResultClass(fillVsMarketable)}">${v314SignedTicks(fillVsMarketable)}</strong><small>${esc(gapText)}</small></div><div><span>Retención del máximo</span><strong class="${effClass}">${effValue}</strong><small>${esc(effText)}</small></div><div><span>Peor quote</span><strong class="${v3110ResultClass(e.worst.exitPnlTicks)}">${v314SignedTicks(e.worst.exitPnlTicks)}</strong><small>${v3110FmtPrice(e.worst.exitQuote,s)}</small></div><div><span>Tiempo en positivo</span><strong>${v3110FmtPct(e.positivePct)}</strong><small>${v315Duration(e.timeToBest)} hasta máximo · ${v315Duration(e.afterBest)} después</small></div></div>`;
...
v30ModeCard=function(){return `<div class="side-bottom"><div class="mini-card mode-card ${v30Ui.modeExpanded?'expanded':''}"><button class="mode-card-toggle" onclick="toggleModeCard()"><span><small>Modo actual</small><strong>V31.10.3</strong></span><b class="mode-card-arrow">${v30Ui.modeExpanded?'▾':'▴'}</b></button><div class="mode-card-detail"><div class="mini-value">${esc(V3110_APP_LABEL)}</div><div class="help">Market Data añade Best Exit / What-if sobre la serie intratrade ya calibrada. El recorrido sigue siendo Last; los benchmarks de salida usan Bid/Ask según el lado de cierre, distinguen evidencia descriptiva de contrafactual y jamás miran después del fill real.</div></div></div></div>`;};
