...
console.log(`State Registry Migration VI -> ${stateRegistryMigration.inventory.names.length} cumulative names (${Object.values(stateRegistryMigration.inventory.batches).map(x=>x.length).join('+')}); explicit entries ${stateRegistryMigration.inventory.before.entries} -> ${stateRegistryMigration.inventory.after.entries}; unique ${stateRegistryMigration.inventory.before.unique} -> ${stateRegistryMigration.inventory.after.unique}`);
