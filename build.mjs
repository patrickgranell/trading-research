...
const bundledApp=safeScript(transformStyleAttrs(stateRegistryMigration.source));
...
const remainingContracts=remainingGlobalContractMap(stateRegistryMigration.source,{runtimeSources:contractMapRuntimeSources,stateActionTransformSource:rawScript('state-action-transform.mjs')});
const replacements=[
...
const bundledRenderDebt=renderDebtInventory(stateRegistryMigration.source);
...
console.log(`State Registry Migration V -> ${stateRegistryMigration.inventory.names.length} cumulative names (${stateRegistryMigration.inventory.batches.batch1.length}+${stateRegistryMigration.inventory.batches.batch2.length}+${stateRegistryMigration.inventory.batches.batch3.length}+${stateRegistryMigration.inventory.batches.batch4.length}+${stateRegistryMigration.inventory.batches.batch5.length}); explicit entries ${stateRegistryMigration.inventory.before.entries} -> ${stateRegistryMigration.inventory.after.entries}; unique ${stateRegistryMigration.inventory.before.unique} -> ${stateRegistryMigration.inventory.after.unique}`);
