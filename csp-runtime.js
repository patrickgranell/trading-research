...
async function trCspProbeHeader(force=false){
  ...
  trCspHeaderPromise=(async()=>{try{const r=await fetch(location.href,{method:'HEAD',cache:'no-store',credentials:'same-origin'}),header=r.headers.get('content-security-policy')||'',ev=trCspEvaluateHeader(header);trCspHeaderState={checked:true,ok:ev.ok,header,error:'',checkedAt:new Date().toISOString(),checks:ev};}catch(e){trCspHeaderState={checked:true,ok:false,header:'',error:e?.message||String(e),checkedAt:new Date().toISOString(),checks:null};}try{if(globalThis.TradingResearchCurrentViewReadContract.current()==='config'&&globalThis.TradingResearchConfigTabStateContract.current()==='data')render();}catch(_){}return trCspHeaderState;})();
...
const trCspModeCard=function(){return `<div class="side-bottom"><div class="mini-card mode-card ${globalThis.TradingResearchModeCardStateReadContract.expanded()?'expanded':''}"><button class="mode-card-toggle" data-tr-onclick="toggleModeCard()"><span><small>Modo actual</small><strong>V31.24</strong></span><b class="mode-card-arrow">${globalThis.TradingResearchModeCardStateReadContract.expanded()?'▾':'▴'}</b></button><div class="mode-card-detail"><div class="mini-value">${globalThis.TradingResearchContentEncodingContract.html(TR_CSP_APP_LABEL)}</div><div class="help">La build define la política CSP objetivo y los gates verifican hashes/directivas. La sonda runtime informa por separado si la cabecera HTTP observada coincide con esa política.</div></div></div></div>`;};
