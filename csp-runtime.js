...
v30ModeCard=function(){return `<div class="side-bottom"><div class="mini-card mode-card ${v30Ui.modeExpanded?'expanded':''}"><button class="mode-card-toggle" data-tr-onclick="toggleModeCard()"><span><small>Modo actual</small><strong>V31.24</strong></span><b class="mode-card-arrow">${v30Ui.modeExpanded?'▾':'▴'}</b></button><div class="mode-card-detail"><div class="mini-value">${esc(TR_CSP_APP_LABEL)}</div><div class="help">La build define la política CSP objetivo y los gates verifican hashes/directivas. La sonda runtime informa por separado si la cabecera HTTP observada coincide con esa política.</div></div></div></div>`;};
