...
html=html.replace('</head>','  <meta name="trading-research-desktop-channel" content="0.3.0" />\n</head>');

const runtime=fs.readFileSync('desktop-native-runtime.js','utf8').replace(/<\/script/gi,'<\\/script');
html=html.replace('</body>',()=>'<script data-tr-desktop-native-storage="0.3.0">'+runtime+'</script>\n</body>');

fs.writeFileSync(indexPath,html);
fs.rmSync(desktopDir+'/_headers',{force:true});

console.log('Prepared offline Desktop 0.3.0 artifact with native recovery runtime -> desktop-dist/');
