...
function trExitSimPanel(ops){
  ...
  const actualVals=settled.map(x=>Number(exitGrossR(byId.get(x.opId))));
  ...
  let body='';
  if(trExitSimState.error)body=`<div class="notice danger"><strong>TP + SL Simulation:</strong> ${esc(trExitSimState.error)}</div>`;
