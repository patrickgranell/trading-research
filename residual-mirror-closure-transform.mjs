...
const LEGACY_DRAG_ATTR=/(?:^|[\s<])(?:ondragstart|ondragend|ondragover|ondrop)\s*=/gm;
...
export function closeResidualDirectMirrors(source){
  ...
  for(const type of DRAG_ATTRS){const re=new RegExp(`(^|[\\s<])on${type}\\s*=`,`gm`);const n=(body.match(re)||[]).length;if(n!==1)throw new Error(`Dynamic Action Closure: on${type} aparece ${n} veces; esperado 1.`);dragAttributeConversions[type]=n;body=body.replace(re,`$1data-tr-on${type}=`);}
