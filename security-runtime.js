...
const trSecurityModeCard=function(){return `<div class="side-bottom"><div class="mini-card mode-card ${globalThis.TradingResearchModeCardStateReadContract.expanded()?'expanded':''}"><button class="mode-card-toggle" data-tr-onclick="toggleModeCard()"><span><small>Modo actual</small><strong>V31.18</strong></span><b class="mode-card-arrow">${globalThis.TradingResearchModeCardStateReadContract.expanded()?'▾':'▴'}</b></button><div class="mode-card-detail"><div class="mini-value">${globalThis.TradingResearchContentEncodingContract.html(TR_SECURITY_APP_LABEL)}</div><div class="help">Boundary de contenido de usuario + FormData en editores principales. Títulos de modal y tokens dinámicos quedan endurecidos contra inyección. La migración completa de handlers inline a delegación de eventos continúa en la siguiente fase; la lógica financiera permanece congelada.</div></div></div></div>`;};
