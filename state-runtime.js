...
const trStateModeCard=function(){return `<div class="side-bottom"><div class="mini-card mode-card ${v30Ui.modeExpanded?'expanded':''}"><button class="mode-card-toggle" data-tr-onclick="toggleModeCard()"><span><small>Modo actual</small><strong>V31.17.1</strong></span><b class="mode-card-arrow">${v30Ui.modeExpanded?'▾':'▴'}</b></button><div class="mode-card-detail"><div class="mini-value">${globalThis.TradingResearchContentEncodingContract.html(TR_STATE_APP_LABEL)}</div><div class="help">Fase estructural 3B3.1a: Imports mantiene el boundary transaccional y ahora normaliza taxonomías/perfiles durables antes de publicar el único commit. Ankora confirma/elimina lotes en un único commit; NinjaTrader Grid agrupa Market Data + sincronización del Journal y el histórico Tick escribe metadata + ticks de forma atómica. Restore y Cloud quedan para las siguientes subfases. La lógica financiera sigue congelada.</div></div></div></div>`;};
