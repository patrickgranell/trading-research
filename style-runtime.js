...
function trStyleResetInventory(){trStyleStats=trStyleBlankStats();trStyleSeenNodes=new WeakSet();trStyleStartedAt=new Date().toISOString();trStyleScanTree(document,'initial');try{window.TradingResearchStyleAttrs?.rescan?.();}catch(_){}try{if(typeof currentView!=='undefined'&&currentView==='config'&&globalThis.TradingResearchConfigTabStateContract.current()==='data')render();}catch(_){}}
