...
function trStyleScanTree(root,kind='inserted'){
  ...
  try{root.querySelectorAll?.('[style]').forEach(el=>trStyleRecordNode(el,kind));}catch(_){}
}
...
const trStyleModeCard=function(){return `<div class="side-bottom"><div class="mini-card mode-card ${globalThis.TradingResearchModeCardStateReadContract.expanded()?'expanded':''}"><button class="mode-card-toggle" data-tr-onclick="toggleModeCard()"><span><small>Modo actual</small><strong>V31.22</strong></span><b class="mode-card-arrow">${globalThis.TradingResearchModeCardStateReadContract.expanded()?'▾':'▴'}</b></button><div class="mode-card-detail"><div class="mini-value">${globalThis.TradingResearchContentEncodingContract.html(TR_STYLE_APP_LABEL)}</div><div class="help">Atributos style históricos transformados en build, CSP style-src-attr none y estilos dinámicos aplicados por una frontera CSSOM controlada. Sin cambios en la lógica financiera.</div></div></div></div>`;};
