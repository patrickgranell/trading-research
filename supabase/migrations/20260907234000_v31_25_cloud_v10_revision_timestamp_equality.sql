-- Trading Research V31.25 · Batch 64 · Cloud V10 revision timestamp equality
-- Fixes false CAS conflicts caused by comparing equivalent timestamptz values as text.
-- PostgreSQL/PostgREST may serialize the same instant differently (space/T, +00/+00:00).
-- Keep the existing atomic transaction, advisory lock, FOR UPDATE and fail-closed conflict semantics.
...
declare
  ...
  v_expected_revision timestamptz := nullif(p_expected_revision, '')::timestamptz;
...
begin
  ...
  elsif v_expected_revision is not null then
    return jsonb_build_object(
      'ok', false,
      'conflict', true,
...
-- Reassert the hardened execution ACL after redefining the SECURITY DEFINER function.
revoke all on function public.apply_trading_workspace(text,jsonb) from public;
revoke all on function public.apply_trading_workspace(text,jsonb) from anon;
revoke all on function public.apply_trading_workspace(text,jsonb) from service_role;
