...
export const TR_UI_REGISTRY_MIGRATION_VERSION='31.23.39';
...
const PRELUDE="const trAppUiActionRegistryV339=(window.TradingResearchActions&&typeof window.TradingResearchActions==='object')?window.TradingResearchActions:(window.TradingResearchActions=Object.create(null));\n";
...
export function migrateUiHandlersToRegistry(source){
  ...
  const body=input.replace(SIMPLE_ASSIGN,(full,list)=>{
    ...
    const registryPart=`Object.assign(trAppUiActionRegistryV339,{${moved.join(',')}});`;
    return `${windowPart}${registryPart}/* V31.23.39 UI registry migration: ${moved.join(', ')} */`;
  });
