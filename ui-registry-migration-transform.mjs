...
export const TR_UI_REGISTRY_MIGRATION_VERSION='31.23.24';
...
const PRELUDE="const trAppUiActionRegistryV324=(window.TradingResearchActions&&typeof window.TradingResearchActions==='object')?window.TradingResearchActions:(window.TradingResearchActions=Object.create(null));\n";
...
export function migrateUiHandlersToRegistry(source){
  ...
  const batch1=new Set(TR_UI_REGISTRY_MIGRATION_BATCH_1),batch2=new Set(TR_UI_REGISTRY_MIGRATION_BATCH_2),batch3=new Set(TR_UI_REGISTRY_MIGRATION_BATCH_3),batch4=new Set(TR_UI_REGISTRY_MIGRATION_BATCH_4),batch5=new Set(TR_UI_REGISTRY_MIGRATION_BATCH_5),batch6=new Set(TR_UI_REGISTRY_MIGRATION_BATCH_6);
  ...
  const body=input.replace(SIMPLE_ASSIGN,(full,list)=>{
    ...
    const registryPart=`Object.assign(trAppUiActionRegistryV324,{${moved.join(',')}});`;
    return `${windowPart}${registryPart}/* V31.23.24 UI registry migration: ${moved.join(', ')} */`;
  });
