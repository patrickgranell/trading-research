...
export const TR_UI_REGISTRY_MIGRATION_VERSION='31.23.30';
...
const PRELUDE="const trAppUiActionRegistryV330=(window.TradingResearchActions&&typeof window.TradingResearchActions==='object')?window.TradingResearchActions:(window.TradingResearchActions=Object.create(null));\n";
...
export function migrateUiHandlersToRegistry(source){
  ...
  const batch1=new Set(TR_UI_REGISTRY_MIGRATION_BATCH_1),batch2=new Set(TR_UI_REGISTRY_MIGRATION_BATCH_2),batch3=new Set(TR_UI_REGISTRY_MIGRATION_BATCH_3),batch4=new Set(TR_UI_REGISTRY_MIGRATION_BATCH_4),batch5=new Set(TR_UI_REGISTRY_MIGRATION_BATCH_5),batch6=new Set(TR_UI_REGISTRY_MIGRATION_BATCH_6),batch7=new Set(TR_UI_REGISTRY_MIGRATION_BATCH_7),batch8=new Set(TR_UI_REGISTRY_MIGRATION_BATCH_8),batch9=new Set(TR_UI_REGISTRY_MIGRATION_BATCH_9),batch10=new Set(TR_UI_REGISTRY_MIGRATION_BATCH_10),batch11=new Set(TR_UI_REGISTRY_MIGRATION_BATCH_11),batch12=new Set(TR_UI_REGISTRY_MIGRATION_BATCH_12);
  const occurrences=Object.fromEntries(TR_UI_REGISTRY_MIGRATION_NAMES.map(n=>[n,0]));
  let touchedBlocks=0,registryEntries=0,batch1Entries=0,batch2Entries=0,batch3Entries=0,batch4Entries=0,batch5Entries=0,batch6Entries=0,batch7Entries=0,batch8Entries=0,batch9Entries=0,batch10Entries=0,batch11Entries=0,batch12Entries=0;
  const body=input.replace(SIMPLE_ASSIGN,(full,list)=>{
    ...
    const registryPart=`Object.assign(trAppUiActionRegistryV330,{${moved.join(',')}});`;
    return `${windowPart}${registryPart}/* V31.23.30 UI registry migration: ${moved.join(', ')} */`;
  });
