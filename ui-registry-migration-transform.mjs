...
export const TR_UI_REGISTRY_MIGRATION_VERSION='31.23.21';
...
const PRELUDE="const trAppUiActionRegistryV321=(window.TradingResearchActions&&typeof window.TradingResearchActions==='object')?window.TradingResearchActions:(window.TradingResearchActions=Object.create(null));\n";
...
export function migrateUiHandlersToRegistry(source){
  ...
  const batch1=new Set(TR_UI_REGISTRY_MIGRATION_BATCH_1),batch2=new Set(TR_UI_REGISTRY_MIGRATION_BATCH_2),batch3=new Set(TR_UI_REGISTRY_MIGRATION_BATCH_3);
  const occurrences=Object.fromEntries(TR_UI_REGISTRY_MIGRATION_NAMES.map(n=>[n,0]));
  let touchedBlocks=0,registryEntries=0,batch1Entries=0,batch2Entries=0,batch3Entries=0;
  const body=input.replace(SIMPLE_ASSIGN,(full,list)=>{
    ...
    const registryPart=`Object.assign(trAppUiActionRegistryV321,{${moved.join(',')}});`;
    return `${windowPart}${registryPart}/* V31.23.21 UI registry migration: ${moved.join(', ')} */`;
  });
