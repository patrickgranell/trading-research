...
export const TR_UI_REGISTRY_MIGRATION_VERSION='31.23.25';
...
const PRELUDE="const trAppUiActionRegistryV325=(window.TradingResearchActions&&typeof window.TradingResearchActions==='object')?window.TradingResearchActions:(window.TradingResearchActions=Object.create(null));\n";
...
export function migrateUiHandlersToRegistry(source){
  ...
  const body=input.replace(SIMPLE_ASSIGN,(full,list)=>{
    ...
    const registryPart=`Object.assign(trAppUiActionRegistryV325,{${moved.join(',')}});`;
    return `${windowPart}${registryPart}/* V31.23.25 UI registry migration: ${moved.join(', ')} */`;
  });
