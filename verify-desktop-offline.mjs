...
if(!html.includes('name="trading-research-desktop-channel" content="0.3.0"'))fail('desktop channel marker is missing.');
if(!html.includes('data-tr-desktop-native-storage="0.3.0"'))fail('Desktop native runtime was not embedded.');
...
if(config.version!=='0.3.0')fail('unexpected Desktop version.');
if(config.build?.frontendDist!=='../desktop-dist')fail('Tauri frontendDist must point at ../desktop-dist.');
if(config.app?.withGlobalTauri!==true)fail('Tauri JS bridge must be enabled.');
...
console.log('Desktop offline gate OK: local frontend, embedded native recovery runtime, no external HTTP(S) assets, NSIS target configured.');
