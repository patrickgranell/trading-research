...
need(overlap.length<=MAX_RUNTIME_NAME_OVERLAP,
  `El proxy lexical app/runtime empeoró en Batch 29: ${overlap.length} > ${MAX_RUNTIME_NAME_OVERLAP}.`);
...
need(directGalleryCalls.length===0,
  `Persisten llamadas ejecutables directas gallery() en runtimes: ${directGalleryCalls.join(', ')}.`);
...
if(fail.length){
  console.error('Gallery View Presentation V2 verification FAILED');
  for(const item of fail)console.error(' - '+item);
  process.exit(1);
}
console.log('Gallery View Presentation V2 verification OK');
console.log(` - legacy lexical runtime name-overlap proxy: ${overlap.length} <= ${MAX_RUNTIME_NAME_OVERLAP}`);
console.log(' - executable direct gallery() runtime calls: 0');
