...
const inv=remainingGlobalContractMap(stage.source,{runtimeSources:runtimes,stateActionTransformSource:fs.readFileSync('state-action-transform.mjs','utf8')});
...
need(inv.remainingUnique===0,`Exports restantes inesperados: ${inv.remainingUnique}`);
need(inv.classified===0,`Clasificados inesperados: ${inv.classified}`);
need(inv.unclassified===0,`Hay ${inv.unclassified} exports sin contrato.`);
need(inv.multiContract===0,`Solapamientos de contrato inesperados: ${inv.multiContract}`);
need(inv.coverage.uiHandler===0,`Cobertura handler inesperada: ${inv.coverage.uiHandler}`);
need(inv.coverage.dynamicAction===0,`Cobertura dinámica inesperada: ${inv.coverage.dynamicAction}`);
need(inv.coverage.stateAction===0,`Cobertura State Action inesperada: ${inv.coverage.stateAction}`);
need(inv.coverage.sameAppGlobalRead===0,`Lecturas globales same-app inesperadas: ${inv.coverage.sameAppGlobalRead}`);
need(inv.coverage.crossRuntimeRead===0,`Quedan ${inv.coverage.crossRuntimeRead} lecturas cross-runtime directas.`);
need(inv.coverage.directGlobalMirror===0,`Mirrors globales directos inesperados: ${inv.coverage.directGlobalMirror}`);
need(!inv.byPrimary['state-action'],`Primary state-action inesperado: ${inv.byPrimary['state-action']}`);
need(!inv.byPrimary['ui-handler'],`Primary ui-handler inesperado: ${inv.byPrimary['ui-handler']}`);
need(!inv.byPrimary['dynamic-action'],`Primary dynamic-action inesperado: ${inv.byPrimary['dynamic-action']}`);
need(inv.names.migrationFrontiers.stateOnly.length===0,`Frontera State inesperada: ${inv.names.migrationFrontiers.stateOnly.length}`);
need(inv.names.migrationFrontiers.handlerOnly.length===0,`Frontera handler-only inesperada: ${inv.names.migrationFrontiers.handlerOnly.length}`);
