...
const contractShape="Object.defineProperty(globalThis,'TradingResearchRemainingUiSnapshotStateReadContract',{value:Object.freeze({operations:()=>typeof opsViewState==='undefined'?undefined:opsViewState,lab:()=>typeof labState==='undefined'?undefined:labState,dashboard:()=>typeof dashboardViewState==='undefined'?undefined:dashboardViewState,exitLab:()=>typeof exitLabState==='undefined'?undefined:exitLabState,bestExit:()=>typeof v3110Ui==='undefined'?undefined:v3110Ui}),writable:false,enumerable:false,configurable:false});";
...
const expectedSnapshotReads=[
  ...
  ["const lab=globalThis.TradingResearchRemainingUiSnapshotStateReadContract.lab();if(lab!==undefined)out.lab=trUiClone(lab);",'lab'],
...
/* Require real structural reduction: five more classic names leave every runtime. */
...
need(runtimeOverlap.length===165,`Batch 57 debe reducir el proxy app/runtime a 165; actual ${runtimeOverlap.length}. Targets presentes=${targets.filter(([name])=>runtimeTokens.has(name)).map(([name])=>name).join(', ')||'ninguno'}.`);
...
console.log(' - snapshot-only bindings migrated: operations, lab, dashboard, exitLab, bestExit');
