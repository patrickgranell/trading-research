...
need(closure.includes("const TR_SOURCE_CONSOLIDATION_PHASE='31.23.15'"),'La tarjeta visible no identifica V31.23.15.');
need(closure.includes('State Registry Migration IV'),'Falta la etiqueta visible de State Registry Migration IV.');
...
need(closure.includes('stateBatch4=Math.max(0,stateMigrated-stateBatch1-stateBatch2-stateBatch3)'),'Source Consolidation no deriva el cuarto lote State desde el total migrado.');
need(closure.includes('sourceBlocks===51&&bundledBlocks===44'),'Source Consolidation no valida el cierre de bloques explícitos.');
need(closure.includes('sourceEntries===375&&bundledEntries===291'),'Source Consolidation no valida la cuarta migración de entries State.');
need(closure.includes('sourceUnique===332&&bundledUnique===254'),'Source Consolidation no valida la cuarta migración de exports State.');
need(closure.includes('stateMigrated===32&&stateBatch1===8&&stateBatch2===8&&stateBatch3===8&&stateBatch4===8'),'Source Consolidation no valida los cuatro lotes State de 8 acciones.');
need(closure.includes('removedBlocks===7&&removedEntries===84&&removedUnique===78&&safeCandidates===0'),'Source Consolidation no conserva la guardia acumulada.');
need(closure.includes('contractRemaining===254&&contractClassified===254&&contractUnclassified===0&&contractMulti===29'),'Source Consolidation no valida cobertura contractual post-migración IV.');
need(closure.includes('primaryState===28&&primaryHandler===223&&primaryDynamic===3'),'Source Consolidation no valida categorías post-migración IV.');
need(closure.includes('stateFrontier===24&&handlerFrontier===221&&crossRuntime===0'),'Source Consolidation no valida fronteras post-migración IV.');
...
for(const name of ['deleteComplianceRule','moveComplianceRule','saveComplianceRule','deleteGoal','saveGoal','deleteTaxonomyAsset','saveTaxonomyAsset','saveVisualReference'])need(closure.includes(name),`Panel V31.23.15 no identifica acción migrada ${name}.`);
...
console.log(' - Visible consolidation phase: V31.23.15 + State Registry Migration IV diagnostics');
