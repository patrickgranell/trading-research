...
need(closure.includes("const TR_SOURCE_CONSOLIDATION_PHASE='31.23.12'"),'La tarjeta visible no identifica V31.23.12.');
need(closure.includes('State Registry Migration I'),'Falta la etiqueta visible de State Registry Migration I.');
...
need(closure.includes('sourceEntries===375&&bundledEntries===316'),'Source Consolidation no valida la migración de entries State.');
need(closure.includes('sourceUnique===332&&bundledUnique===278'),'Source Consolidation no valida la migración de exports State.');
need(closure.includes('stateMigrated===8'),'Source Consolidation no valida las 8 acciones State migradas.');
need(closure.includes('removedBlocks===7&&removedEntries===59&&removedUnique===54&&safeCandidates===0'),'Source Consolidation no conserva la guardia acumulada.');
need(closure.includes('contractRemaining===278&&contractClassified===278&&contractUnclassified===0&&contractMulti===53'),'Source Consolidation no valida cobertura contractual post-migración.');
need(closure.includes('primaryState===52&&primaryHandler===223&&primaryDynamic===3'),'Source Consolidation no valida categorías post-migración.');
need(closure.includes('stateFrontier===48&&handlerFrontier===221&&crossRuntime===0'),'Source Consolidation no valida fronteras post-migración.');
...
for(const name of ['setOpsUnit','setOpsBasis','toggleOpsDay','toggleOpsModule','resetOpsFilters','setOpsQuickPeriod','setOpsDimension','applyHeatCell'])need(closure.includes(name),`Panel V31.23.12 no identifica acción migrada ${name}.`);
...
console.log(' - Visible consolidation phase: V31.23.12 + State Registry Migration I diagnostics');
