...
need(pkg.version==='31.18.0',`Versión inesperada ${pkg.version}`);
need(index.includes('<script src="security-runtime.js"></script>'),'index.html no carga security-runtime.js al final.');
need(index.includes('trading-research-source-version" content="31.18.0"'),'source-version no es 31.18.0.');
...
need(app.includes("inlineUriToken(meta.caption||meta.name||meta.label||'Imagen')"),'Caption de imagen no usa token seguro en el handler legacy.');
...
need(app.includes('const body=`<form onsubmit="return false"><div class="trade-context-strip">'),'Diario emocional no está envuelto en form.');
need(app.includes('const body=`<form onsubmit="return false"><div class="form-section"><h4>Límites diarios</h4>'),'Gestión de riesgo no está envuelta en form.');
...
need(sec.includes('inlineHandlersLegacy:true')&&sec.includes('strictCspReady:false'),'La deuda de handlers inline/CSP no está declarada explícitamente.');
...
const inlineHandlers=(app.match(/\bon(?:click|change|input|submit)=/g)||[]).length;
...
console.log(' - Dynamic URI tokens in legacy inline handlers: apostrophe-safe');
console.log(' - Core FormData boundaries: 7/7');
console.log(` - Legacy inline handlers remaining (explicit debt, not hidden): ${inlineHandlers}`);
console.log(' - Strict CSP/event-delegation completion: intentionally deferred');
