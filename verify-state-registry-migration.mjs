...
import {migrateStateActionsToRegistry,TR_STATE_REGISTRY_MIGRATION_NAMES,TR_STATE_REGISTRY_MIGRATION_BATCH_1,TR_STATE_REGISTRY_MIGRATION_BATCH_2,TR_STATE_REGISTRY_MIGRATION_BATCH_3,TR_STATE_REGISTRY_MIGRATION_VERSION} from './state-registry-migration-transform.mjs';
...
if(inv.after.unique!==262)throw new Error(`Exports window únicos tras tres lotes: ${inv.after.unique}; se esperaban 262.`);
...
if(mapAfter.remainingUnique!==262||mapAfter.classified!==262||mapAfter.unclassified!==0)throw new Error(`Mapa contractual post-migración inesperado: ${mapAfter.classified}/${mapAfter.remainingUnique}, sin clasificar ${mapAfter.unclassified}.`);
if((mapAfter.byPrimary['state-action']||0)!==36||(mapAfter.byPrimary['ui-handler']||0)!==223||(mapAfter.byPrimary['dynamic-action']||0)!==3)throw new Error(`Primarios post-migración inesperados: State ${mapAfter.byPrimary['state-action']||0}, UI ${mapAfter.byPrimary['ui-handler']||0}, dinámico ${mapAfter.byPrimary['dynamic-action']||0}.`);
if(mapAfter.names.migrationFrontiers.stateOnly.length!==32)throw new Error(`Frontera State post-migración inesperada: ${mapAfter.names.migrationFrontiers.stateOnly.length}.`);
...
if(!migrated.source.includes('V31.23.14 State registry migration:'))throw new Error('Falta marcador V31.23.14 en el bundle migrado.');
console.log('State Registry Migration verification OK');
console.log(` - Cumulative State actions migrated: ${TR_STATE_REGISTRY_MIGRATION_NAMES.length} (${TR_STATE_REGISTRY_MIGRATION_BATCH_1.length}+${TR_STATE_REGISTRY_MIGRATION_BATCH_2.length}+${TR_STATE_REGISTRY_MIGRATION_BATCH_3.length})`);
...
console.log(` - Registry publications: ${inv.registryEntries} total; batch I ${inv.batch1Entries}; batch II ${inv.batch2Entries}; batch III ${inv.batch3Entries}`);
...
console.log(` - Batch III migrated: ${TR_STATE_REGISTRY_MIGRATION_BATCH_3.join(', ')}`);
