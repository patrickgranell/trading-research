 ...
 * Plan read, content encoding, Exit Lab presentation, then Form Boundary.
 * Removing all four suffixes must reproduce the audited app.js exactly; this is
 * intentionally stronger than replacing the historical baseline hash. */
...
const architecturalContractSuffix=planReadContractSuffix+contentEncodingContractSuffix+exitPresentationContractSuffix+formBoundaryContractSuffix;
if(!app.endsWith(architecturalContractSuffix)){
  fail.push('app.js no termina exactamente con la cadena contractual Plan + Content Encoding + Exit Presentation + Form Boundary permitida.');
}else{
...
console.log(' - app.js: audited V31.25 source preserved byte-for-byte beneath exact terminal Plan-read + content-encoding + Exit-presentation + Form-boundary contracts');
