...
if(inv.after.blocks!==25)throw new Error(`Bloques window tras UI XX: ${inv.after.blocks}; se esperaban 25.`);
if(inv.registryEntries!==172||inv.batch1Entries!==8||inv.batch2Entries!==8||inv.batch3Entries!==11||inv.batch4Entries!==9||inv.batch5Entries!==8||inv.batch6Entries!==8||inv.batch7Entries!==9||inv.batch8Entries!==8||inv.batch9Entries!==9||inv.batch10Entries!==8||inv.batch11Entries!==8||inv.batch12Entries!==8||inv.batch13Entries!==10||inv.batch14Entries!==8||inv.batch15Entries!==9||inv.batch16Entries!==9||inv.batch17Entries!==8||inv.batch18Entries!==8||inv.batch19Entries!==8||inv.batch20Entries!==10)throw new Error(`Publicaciones UI inesperadas: total ${inv.registryEntries}, lote XX ${inv.batch20Entries}.`);
if(inv.after.entries!==90)throw new Error(`Entries window tras UI XX: ${inv.after.entries}; se esperaban 90.`);
if(inv.after.unique!==70)throw new Error(`Exports window únicos tras UI XX: ${inv.after.unique}; se esperaban 70.`);
if(mapAfter.remainingUnique!==70||mapAfter.classified!==70||mapAfter.unclassified!==0)throw new Error(`Mapa contractual UI post-migración inesperado: ${mapAfter.classified}/${mapAfter.remainingUnique}, sin clasificar ${mapAfter.unclassified}.`);
if((mapAfter.byPrimary['state-action']||0)!==4||(mapAfter.byPrimary['ui-handler']||0)!==63||(mapAfter.byPrimary['dynamic-action']||0)!==3)throw new Error(`Primarios UI post-migración inesperados: State ${mapAfter.byPrimary['state-action']||0}, UI ${mapAfter.byPrimary['ui-handler']||0}, dinámico ${mapAfter.byPrimary['dynamic-action']||0}.`);
if(mapAfter.names.migrationFrontiers.stateOnly.length!==0)throw new Error('La frontera State se reabrió durante UI XX.');
if(mapAfter.names.migrationFrontiers.handlerOnly.length!==61)throw new Error(`Frontera handler-only tras UI XX: ${mapAfter.names.migrationFrontiers.handlerOnly.length}; se esperaban 61.`);
if(mapAfter.coverage.crossRuntimeRead!==0)throw new Error(`UI XX reabrió ${mapAfter.coverage.crossRuntimeRead} lecturas cross-runtime.`);
...
console.log('UI Registry Migration XX verification OK');
