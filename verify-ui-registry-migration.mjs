...
import {migrateUiHandlersToRegistry,TR_UI_REGISTRY_MIGRATION_NAMES,TR_UI_REGISTRY_MIGRATION_BATCH_1,TR_UI_REGISTRY_MIGRATION_BATCH_2,TR_UI_REGISTRY_MIGRATION_BATCH_3,TR_UI_REGISTRY_MIGRATION_VERSION} from './ui-registry-migration-transform.mjs';
...
if(mapAfter.names.migrationFrontiers.stateOnly.length!==0)throw new Error('La frontera State se reabrió durante UI III.');
if(mapAfter.names.migrationFrontiers.handlerOnly.length!==197)throw new Error(`Frontera handler-only tras UI III: ${mapAfter.names.migrationFrontiers.handlerOnly.length}; se esperaban 197.`);
if(mapAfter.coverage.crossRuntimeRead!==0)throw new Error(`UI III reabrió ${mapAfter.coverage.crossRuntimeRead} lecturas cross-runtime.`);
...
console.log('UI Registry Migration III verification OK');
console.log(` - UI handlers migrated: ${TR_UI_REGISTRY_MIGRATION_NAMES.length} (${TR_UI_REGISTRY_MIGRATION_BATCH_1.length}+${TR_UI_REGISTRY_MIGRATION_BATCH_2.length}+${TR_UI_REGISTRY_MIGRATION_BATCH_3.length})`);
...
console.log(` - Registry publications: ${inv.registryEntries}; batches ${inv.batch1Entries}/${inv.batch2Entries}/${inv.batch3Entries}`);
...
console.log(` - Batch UI III: ${TR_UI_REGISTRY_MIGRATION_BATCH_3.join(', ')}`);
