...
if(inv.after.blocks!==23)throw new Error(`Bloques window tras UI XXI: ${inv.after.blocks}; se esperaban 23.`);
if(inv.registryEntries!==182||inv.batch1Entries!==8||inv.batch2Entries!==8||inv.batch3Entries!==11||inv.batch4Entries!==9||inv.batch5Entries!==8||inv.batch6Entries!==8||inv.batch7Entries!==9||inv.batch8Entries!==8||inv.batch9Entries!==9||inv.batch10Entries!==8||inv.batch11Entries!==8||inv.batch12Entries!==8||inv.batch13Entries!==10||inv.batch14Entries!==8||inv.batch15Entries!==9||inv.batch16Entries!==9||inv.batch17Entries!==8||inv.batch18Entries!==8||inv.batch19Entries!==8||inv.batch20Entries!==10||inv.batch21Entries!==10)throw new Error(`Publicaciones UI inesperadas: total ${inv.registryEntries}, lote XXI ${inv.batch21Entries}.`);
if(inv.after.entries!==80)throw new Error(`Entries window tras UI XXI: ${inv.after.entries}; se esperaban 80.`);
if(inv.after.unique!==62)throw new Error(`Exports window únicos tras UI XXI: ${inv.after.unique}; se esperaban 62.`);
if(mapAfter.remainingUnique!==62||mapAfter.classified!==62||mapAfter.unclassified!==0)throw new Error(`Mapa contractual UI post-migración inesperado: ${mapAfter.classified}/${mapAfter.remainingUnique}, sin clasificar ${mapAfter.unclassified}.`);
if((mapAfter.byPrimary['state-action']||0)!==4||(mapAfter.byPrimary['ui-handler']||0)!==55||(mapAfter.byPrimary['dynamic-action']||0)!==3)throw new Error(`Primarios UI post-migración inesperados: State ${mapAfter.byPrimary['state-action']||0}, UI ${mapAfter.byPrimary['ui-handler']||0}, dinámico ${mapAfter.byPrimary['dynamic-action']||0}.`);
if(mapAfter.names.migrationFrontiers.stateOnly.length!==0)throw new Error('La frontera State se reabrió durante UI XXI.');
if(mapAfter.names.migrationFrontiers.handlerOnly.length!==53)throw new Error(`Frontera handler-only tras UI XXI: ${mapAfter.names.migrationFrontiers.handlerOnly.length}; se esperaban 53.`);
if(mapAfter.coverage.crossRuntimeRead!==0)throw new Error(`UI XXI reabrió ${mapAfter.coverage.crossRuntimeRead} lecturas cross-runtime.`);
...
console.log('UI Registry Migration XXI verification OK');
...
console.log(` - Registry publications: ${inv.registryEntries}; batch XXI ${inv.batch21Entries}`);
...
console.log(` - Batch UI XXI: ${TR_UI_REGISTRY_MIGRATION_BATCH_21.join(', ')}`);
