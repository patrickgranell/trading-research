...
import {migrateUiHandlersToRegistry,TR_UI_REGISTRY_MIGRATION_NAMES,TR_UI_REGISTRY_MIGRATION_BATCH_1,TR_UI_REGISTRY_MIGRATION_BATCH_2,TR_UI_REGISTRY_MIGRATION_BATCH_3,TR_UI_REGISTRY_MIGRATION_BATCH_4,TR_UI_REGISTRY_MIGRATION_BATCH_5,TR_UI_REGISTRY_MIGRATION_VERSION} from './ui-registry-migration-transform.mjs';
...
if(mapAfter.remainingUnique!==190||mapAfter.classified!==190||mapAfter.unclassified!==0)throw new Error(`Mapa contractual UI post-migración inesperado: ${mapAfter.classified}/${mapAfter.remainingUnique}, sin clasificar ${mapAfter.unclassified}.`);
if((mapAfter.byPrimary['state-action']||0)!==4||(mapAfter.byPrimary['ui-handler']||0)!==183||(mapAfter.byPrimary['dynamic-action']||0)!==3)throw new Error(`Primarios UI post-migración inesperados: State ${mapAfter.byPrimary['state-action']||0}, UI ${mapAfter.byPrimary['ui-handler']||0}, dinámico ${mapAfter.byPrimary['dynamic-action']||0}.`);
if(mapAfter.names.migrationFrontiers.stateOnly.length!==0)throw new Error('La frontera State se reabrió durante UI V.');
if(mapAfter.names.migrationFrontiers.handlerOnly.length!==181)throw new Error(`Frontera handler-only tras UI V: ${mapAfter.names.migrationFrontiers.handlerOnly.length}; se esperaban 181.`);
if(mapAfter.coverage.crossRuntimeRead!==0)throw new Error(`UI V reabrió ${mapAfter.coverage.crossRuntimeRead} lecturas cross-runtime.`);
...
console.log('UI Registry Migration V verification OK');
console.log(` - UI handlers migrated: ${TR_UI_REGISTRY_MIGRATION_NAMES.length} (${TR_UI_REGISTRY_MIGRATION_BATCH_1.length}+${TR_UI_REGISTRY_MIGRATION_BATCH_2.length}+${TR_UI_REGISTRY_MIGRATION_BATCH_3.length}+${TR_UI_REGISTRY_MIGRATION_BATCH_4.length}+${TR_UI_REGISTRY_MIGRATION_BATCH_5.length})`);
...
console.log(` - Registry publications: ${inv.registryEntries}; batches ${inv.batch1Entries}/${inv.batch2Entries}/${inv.batch3Entries}/${inv.batch4Entries}/${inv.batch5Entries}`);
...
console.log(` - Batch UI V: ${TR_UI_REGISTRY_MIGRATION_BATCH_5.join(', ')}`);
