...
import {migrateUiHandlersToRegistry,TR_UI_REGISTRY_MIGRATION_NAMES,TR_UI_REGISTRY_MIGRATION_BATCH_1,TR_UI_REGISTRY_MIGRATION_VERSION} from './ui-registry-migration-transform.mjs';
...
if(TR_UI_REGISTRY_MIGRATION_VERSION!=='31.23.19')throw new Error(`Versión UI inesperada: ${TR_UI_REGISTRY_MIGRATION_VERSION}`);
if(TR_UI_REGISTRY_MIGRATION_BATCH_1.length!==8||TR_UI_REGISTRY_MIGRATION_NAMES.length!==8)throw new Error('El lote UI I debe contener exactamente 8 acciones.');
...
if(inv.after.blocks>inv.before.blocks)throw new Error(`La migración UI aumentó bloques window: ${inv.before.blocks} -> ${inv.after.blocks}.`);
...
if(inv.after.unique!==222)throw new Error(`Exports window únicos tras UI I: ${inv.after.unique}; se esperaban 222.`);
if(mapAfter.remainingUnique!==222||mapAfter.classified!==222||mapAfter.unclassified!==0)throw new Error(`Mapa contractual UI post-migración inesperado: ${mapAfter.classified}/${mapAfter.remainingUnique}, sin clasificar ${mapAfter.unclassified}.`);
if((mapAfter.byPrimary['state-action']||0)!==4||(mapAfter.byPrimary['ui-handler']||0)!==215||(mapAfter.byPrimary['dynamic-action']||0)!==3)throw new Error(`Primarios UI post-migración inesperados: State ${mapAfter.byPrimary['state-action']||0}, UI ${mapAfter.byPrimary['ui-handler']||0}, dinámico ${mapAfter.byPrimary['dynamic-action']||0}.`);
if(mapAfter.names.migrationFrontiers.stateOnly.length!==0)throw new Error('La frontera State se reabrió durante UI I.');
if(mapAfter.names.migrationFrontiers.handlerOnly.length!==213)throw new Error(`Frontera handler-only tras UI I: ${mapAfter.names.migrationFrontiers.handlerOnly.length}; se esperaban 213.`);
if(mapAfter.coverage.crossRuntimeRead!==0)throw new Error(`UI I reabrió ${mapAfter.coverage.crossRuntimeRead} lecturas cross-runtime.`);
...
console.log('UI Registry Migration I verification OK');
...
console.log(` - Registry publications: ${inv.registryEntries}`);
...
console.log(` - Batch UI I: ${TR_UI_REGISTRY_MIGRATION_BATCH_1.join(', ')}`);
