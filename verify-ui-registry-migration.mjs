...
if(TR_UI_REGISTRY_MIGRATION_VERSION!=='31.23.31')throw new Error(`Versión UI inesperada: ${TR_UI_REGISTRY_MIGRATION_VERSION}`);
if(TR_UI_REGISTRY_MIGRATION_BATCH_1.length!==8||TR_UI_REGISTRY_MIGRATION_BATCH_2.length!==8||TR_UI_REGISTRY_MIGRATION_BATCH_3.length!==8||TR_UI_REGISTRY_MIGRATION_BATCH_4.length!==8||TR_UI_REGISTRY_MIGRATION_BATCH_5.length!==8||TR_UI_REGISTRY_MIGRATION_BATCH_6.length!==8||TR_UI_REGISTRY_MIGRATION_BATCH_7.length!==8||TR_UI_REGISTRY_MIGRATION_BATCH_8.length!==8||TR_UI_REGISTRY_MIGRATION_BATCH_9.length!==8||TR_UI_REGISTRY_MIGRATION_BATCH_10.length!==8||TR_UI_REGISTRY_MIGRATION_BATCH_11.length!==8||TR_UI_REGISTRY_MIGRATION_BATCH_12.length!==8||TR_UI_REGISTRY_MIGRATION_BATCH_13.length!==8||TR_UI_REGISTRY_MIGRATION_NAMES.length!==104)throw new Error('Los lotes UI I/XIII deben contener 8 acciones cada uno.');
...
if(inv.after.blocks!==30)throw new Error(`Bloques window tras UI XIII: ${inv.after.blocks}; se esperaban 30.`);
if(inv.registryEntries!==112||inv.batch1Entries!==8||inv.batch2Entries!==8||inv.batch3Entries!==11||inv.batch4Entries!==9||inv.batch5Entries!==8||inv.batch6Entries!==8||inv.batch7Entries!==9||inv.batch8Entries!==8||inv.batch9Entries!==9||inv.batch10Entries!==8||inv.batch11Entries!==8||inv.batch12Entries!==8||inv.batch13Entries!==10)throw new Error(`Publicaciones UI inesperadas: total ${inv.registryEntries}, lote XIII ${inv.batch13Entries}.`);
if(inv.after.entries!==150)throw new Error(`Entries window tras UI XIII: ${inv.after.entries}; se esperaban 150.`);
if(inv.after.unique!==126)throw new Error(`Exports window únicos tras UI XIII: ${inv.after.unique}; se esperaban 126.`);
if(mapAfter.remainingUnique!==126||mapAfter.classified!==126||mapAfter.unclassified!==0)throw new Error(`Mapa contractual UI post-migración inesperado: ${mapAfter.classified}/${mapAfter.remainingUnique}, sin clasificar ${mapAfter.unclassified}.`);
if((mapAfter.byPrimary['state-action']||0)!==4||(mapAfter.byPrimary['ui-handler']||0)!==119||(mapAfter.byPrimary['dynamic-action']||0)!==3)throw new Error(`Primarios UI post-migración inesperados: State ${mapAfter.byPrimary['state-action']||0}, UI ${mapAfter.byPrimary['ui-handler']||0}, dinámico ${mapAfter.byPrimary['dynamic-action']||0}.`);
if(mapAfter.names.migrationFrontiers.stateOnly.length!==0)throw new Error('La frontera State se reabrió durante UI XIII.');
if(mapAfter.names.migrationFrontiers.handlerOnly.length!==117)throw new Error(`Frontera handler-only tras UI XIII: ${mapAfter.names.migrationFrontiers.handlerOnly.length}; se esperaban 117.`);
if(mapAfter.coverage.crossRuntimeRead!==0)throw new Error(`UI XIII reabrió ${mapAfter.coverage.crossRuntimeRead} lecturas cross-runtime.`);
...
console.log('UI Registry Migration XIII verification OK');
...
console.log(` - Registry publications: ${inv.registryEntries}; batch XIII ${inv.batch13Entries}`);
...
console.log(` - Batch UI XIII: ${TR_UI_REGISTRY_MIGRATION_BATCH_13.join(', ')}`);
