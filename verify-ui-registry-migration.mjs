...
if(inv.after.blocks!==26)throw new Error(`Bloques window tras UI XVII: ${inv.after.blocks}; se esperaban 26.`);
if(inv.registryEntries!==146||inv.batch1Entries!==8||inv.batch2Entries!==8||inv.batch3Entries!==11||inv.batch4Entries!==9||inv.batch5Entries!==8||inv.batch6Entries!==8||inv.batch7Entries!==9||inv.batch8Entries!==8||inv.batch9Entries!==9||inv.batch10Entries!==8||inv.batch11Entries!==8||inv.batch12Entries!==8||inv.batch13Entries!==10||inv.batch14Entries!==8||inv.batch15Entries!==9||inv.batch16Entries!==9||inv.batch17Entries!==8)throw new Error(`Publicaciones UI inesperadas: total ${inv.registryEntries}, lote XVII ${inv.batch17Entries}.`);
if(inv.after.entries!==116)throw new Error(`Entries window tras UI XVII: ${inv.after.entries}; se esperaban 116.`);
if(inv.after.unique!==94)throw new Error(`Exports window únicos tras UI XVII: ${inv.after.unique}; se esperaban 94.`);
if(mapAfter.remainingUnique!==94||mapAfter.classified!==94||mapAfter.unclassified!==0)throw new Error(`Mapa contractual UI post-migración inesperado: ${mapAfter.classified}/${mapAfter.remainingUnique}, sin clasificar ${mapAfter.unclassified}.`);
if((mapAfter.byPrimary['state-action']||0)!==4||(mapAfter.byPrimary['ui-handler']||0)!==87||(mapAfter.byPrimary['dynamic-action']||0)!==3)throw new Error(`Primarios UI post-migración inesperados: State ${mapAfter.byPrimary['state-action']||0}, UI ${mapAfter.byPrimary['ui-handler']||0}, dinámico ${mapAfter.byPrimary['dynamic-action']||0}.`);
if(mapAfter.names.migrationFrontiers.stateOnly.length!==0)throw new Error('La frontera State se reabrió durante UI XVII.');
if(mapAfter.names.migrationFrontiers.handlerOnly.length!==85)throw new Error(`Frontera handler-only tras UI XVII: ${mapAfter.names.migrationFrontiers.handlerOnly.length}; se esperaban 85.`);
if(mapAfter.coverage.crossRuntimeRead!==0)throw new Error(`UI XVII reabrió ${mapAfter.coverage.crossRuntimeRead} lecturas cross-runtime.`);
...
console.log('UI Registry Migration XVII verification OK');
...
console.log(` - Registry publications: ${inv.registryEntries}; batch XVII ${inv.batch17Entries}`);
...
console.log(` - Batch UI XVII: ${TR_UI_REGISTRY_MIGRATION_BATCH_17.join(', ')}`);
