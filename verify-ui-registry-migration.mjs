...
if(inv.after.blocks!==30)throw new Error(`Bloques window tras UI XV: ${inv.after.blocks}; se esperaban 30.`);
if(inv.registryEntries!==129||inv.batch1Entries!==8||inv.batch2Entries!==8||inv.batch3Entries!==11||inv.batch4Entries!==9||inv.batch5Entries!==8||inv.batch6Entries!==8||inv.batch7Entries!==9||inv.batch8Entries!==8||inv.batch9Entries!==9||inv.batch10Entries!==8||inv.batch11Entries!==8||inv.batch12Entries!==8||inv.batch13Entries!==10||inv.batch14Entries!==8||inv.batch15Entries!==9)throw new Error(`Publicaciones UI inesperadas: total ${inv.registryEntries}, lote XV ${inv.batch15Entries}.`);
if(inv.after.entries!==133)throw new Error(`Entries window tras UI XV: ${inv.after.entries}; se esperaban 133.`);
if(inv.after.unique!==110)throw new Error(`Exports window únicos tras UI XV: ${inv.after.unique}; se esperaban 110.`);
if(mapAfter.remainingUnique!==110||mapAfter.classified!==110||mapAfter.unclassified!==0)throw new Error(`Mapa contractual UI post-migración inesperado: ${mapAfter.classified}/${mapAfter.remainingUnique}, sin clasificar ${mapAfter.unclassified}.`);
if((mapAfter.byPrimary['state-action']||0)!==4||(mapAfter.byPrimary['ui-handler']||0)!==103||(mapAfter.byPrimary['dynamic-action']||0)!==3)throw new Error(`Primarios UI post-migración inesperados: State ${mapAfter.byPrimary['state-action']||0}, UI ${mapAfter.byPrimary['ui-handler']||0}, dinámico ${mapAfter.byPrimary['dynamic-action']||0}.`);
if(mapAfter.names.migrationFrontiers.stateOnly.length!==0)throw new Error('La frontera State se reabrió durante UI XV.');
if(mapAfter.names.migrationFrontiers.handlerOnly.length!==101)throw new Error(`Frontera handler-only tras UI XV: ${mapAfter.names.migrationFrontiers.handlerOnly.length}; se esperaban 101.`);
if(mapAfter.coverage.crossRuntimeRead!==0)throw new Error(`UI XV reabrió ${mapAfter.coverage.crossRuntimeRead} lecturas cross-runtime.`);
...
console.log('UI Registry Migration XV verification OK');
...
console.log(` - Registry publications: ${inv.registryEntries}; batch XV ${inv.batch15Entries}`);
...
console.log(` - Batch UI XV: ${TR_UI_REGISTRY_MIGRATION_BATCH_15.join(', ')}`);
